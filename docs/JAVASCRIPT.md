...
- **`fn* name() { yield v }`** — generatorfunktion (self-host + **kab-only** Kab-VM). Anrop **`name()`** ger iterator. **`yield* iterable`** delegerar; delegatens **`return`**-värde blir **`yield*`**-uttrycket. **`return expr`** blir **`value`** på sista **`.next()`** med **`done: true`**. **`try` / `catch` / `finally`** i **`fn*`** (även nästlad + yttre **`finally`**) körs vid **`.next()`**, **`.return(v)`**, **`.throw(e)`** och **send** (`g.next(v)` in i `let x = yield`). **`async fn*`** är **inte** samma vägg som synk-**`fn*`**.
- **`async fn* name() { yield v }`** — async generator (host-paritet; inte SH6-deepen). **`yield* asyncIterable`** delegerar över async/sync; **`await`** i kroppen; **`.next()` / `.return()` / `.throw()`** returnerar **`Promise<{ value, done }>`**; under aktiv **`yield*`** vidarebefordras abrupt completion till delegaten; **`for await…of`**.
...
- **`it.return(value?)`** / **`it.throw(reason?)`** — tidig stängning. Synk-**`fn*`** (kab-only): **`.throw(e)`** i `try`-`yield` kör inner **`finally`** sedan yttre **`catch`** om den finns; **`.throw`** i en **`finally`-`yield`** fångas inte av **samma** `try`:s **`catch`**. **`.return(v)`** kör alla täckande **`finally`** (nästlade + yttre) innan completion **`v`**. Under **`yield*`** vidarebefordras **`.return()`** / **`.throw()`** till delegaten. **`break`** / **`return`** / **`raise`** i **`for…of`** anropar **`iterator_close`**. **`fn* gen()`**: **`gen[Symbol.iterator]()`** returnerar generatorn själv.
