...
| **F10** | **AOT + PGO** — warmed image; profilstyrd JIT/AOT | kallstart + steady-state gates | ✅ subset: x64/arm64 ship/ret-op/nop-op (load+verify)/zero-op (load+verify)/one-op (load+verify)/add-op (load+verify)/sub-op (load+verify)/text, layout, and native (one+add+nop+ret)/rodata/data plus write/read bind; native machine-code deepen |
...
**Nästa steg (bara `.kab`):** F10 text that prefixes one+add+sub+nop+ret — `nollAotReady` förblir false. **Radera inte `src/`.** Inga nya `src/*.rs`.
