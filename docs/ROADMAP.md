...
**Aktivt fokus (2026-08):** P6b — fas-profil + `emit_impl` AccAdd-hotpath (recurse); `serialize_body` densified; list stays 5 until <10s; GP6n WebXR input sources. **P/GP0–GP5** + **GP6a–n subset** + **SC0–SC7** + **DX0–DX7** + SIM/DATA/IOT/APP MVP subset landad.
...
| **P6** | **Self-host compile-tid** — tömma H6e skip-list; snabbare parse/emit; incremental `.kbc`; committed `self_host/seed/*.kbc` | ✅ **seed-only product path** (5 leaves + fingerprint gates). **P6b** empty list 📋 när budget <10s — leaf densify + `emit_impl` AccAdd recurse hotpath (list stays 5; see `self_host/seed/README.md`) |
