...
| **Nästa kod** | Byta `http_fetch_async` till Kab-TLS (inte rustls). Hex-TCP tills `tcp_read` bär råa TLS-bytes. |
...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 Kab TLS application-data/AES-GCM unwrap med recordsäker hex-TCP-loopback + query Location (`sh23_crypto_http_fetch_eval_smoke`); rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** byt `http_fetch_async` från rustls till Kab-TLS när `tcp_read` kan bära råa TLS-bytes. |
