...
**SH6 = KLAR** mot stoppkriteriet (appar + using). **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** `.kbc`/`.kbcb` disk-eval. **SH20** JSON parse/stringify. **SH21** join+norm+write+read. **SH22 deepen:** `sqlExecOk` INSERT+SELECT eq (`sh22_sql_exec_smoke`). **Nästa: SH23 krypto** (riktig eval, inte flagg-kloner). Inte async fn*. Inte fler sqlIs*-kloner.
