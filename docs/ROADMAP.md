...
| **Nästa kod** | rustls-host är kvar (`cryptoHostDeleteOk` false). Kab TLS 1.3 SAN iPAddress length 11 rejected mot rustls P-256-peer `:28457` (`sh23_crypto_tls13_peer_n11_eval_smoke`). Cheap smokes importerar inte crypto. Nästa gate: Kab TLS 1.3 SAN iPAddress length 12 rejected mot rustls-peer (inte radera rustls). |
