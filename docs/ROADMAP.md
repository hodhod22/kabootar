...
| **F10** | **AOT + PGO** — warmed image; profilstyrd JIT/AOT | kallstart + steady-state gates | ✅ subset: warm/PGO/image/cold/steady + native-image policy + `aotSealOk` + `aotCacheKeyOk` + `aotLayoutOk` + `aotTargetOk`; native emit deepen |
