...
| **F10** | **AOT + PGO** — warmed image; profilstyrd JIT/AOT | kallstart + steady-state gates | ✅ subset: x64/arm64 ship/ret-op/text/rodata/data/layout plus emit/load native round-trip, load reject, and verify native round/reject/arm64; native machine-code deepen |
...
| **SH5** | **Reverse-densify** — slå ihop tunna shards; compile-DAG **< 80** | SH0 ner; leaf ≤10 s | ✅ deepen: `parser_expr` (unary/mul/add/rel/compare); `parser_hooks`+util; `emit_fn_scope`+fn_body |
...
**Nästa steg (bara `.kab`):** F10 native emit deepen — `nollAotReady` förblir false. **Radera inte `src/`.** Inga nya `src/*.rs`.
