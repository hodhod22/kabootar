...
**Generator-SH6 = KLAR.** **using class `close()` writeback = KLAR** (`sh6_self_host_using_class_close_writeback_ok` → **1**). Inga fler generator-tester. **async fn* är låst.** **async =>** skjuts upp (inte SH6-blockerare).
...
**SH6 = KLAR** mot stoppkriteriet (appar + using). **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** `.kbc`/`.kbcb` disk-eval. **SH20** JSON parse/stringify. **SH21** join+norm+write+read. **SH22 deepen:** `sqlExecOk` INSERT+SELECT eq (`sh22_sql_exec_smoke`). **SH23 SHA/HMAC/PRF/handshake/GCM/X25519/HTTPS-GET + RSA/ECDSA cert-verify + https URL-parse GET + TLS app-data record + HTTP 200 unwrap + http_fetch URL/record + 302 Location follow + :443 port + chunked decode + multi TLS app-data records + POST body + JSON POST + PUT/PATCH + HEAD/DELETE + OPTIONS/CONNECT + TRACE + UA/Bearer i Kab (`sh23_crypto_sha_eval_smoke`, `sh23_crypto_hmac_eval_smoke`, `sh23_crypto_tls_prf_eval_smoke`, `sh23_crypto_tls_hs_eval_smoke`, `sh23_crypto_tls_gcm_eval_smoke`, `sh23_crypto_x25519_eval_smoke`, `sh23_crypto_tls_https_eval_smoke`, `sh23_crypto_tls_cert_eval_smoke`, `sh23_crypto_tls_ecdsa_eval_smoke`, `sh23_crypto_http_get_eval_smoke`, `sh23_crypto_http_resp_eval_smoke`, `sh23_crypto_http_fetch_eval_smoke`). rustls-host kvar. Nästa: rustls-delete när `http_fetch` går via Kab** (inte fler flagg-kloner). Inte async fn*. Inte fler sqlIs*-kloner.
