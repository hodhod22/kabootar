...
**Aktivt fokus (2026-08):** P6b hoist IR tables to `serialize_defs` + `outTag`/`sLine` AccAdd; GP6n `xrLocateHandJoints` stub EXT locate → live buffers; skip-list stays 5 until <10s. **P/GP0–GP5** + **GP6a–n subset** + **SC0–SC7** + **DX0–DX7** + SIM/DATA/IOT/APP MVP subset landad.
...
| **P6** | **Self-host compile-tid** — tömma H6e skip-list; snabbare parse/emit; incremental `.kbc`; committed `self_host/seed/*.kbc` | ✅ **seed-only**. **P6b** 📋 <10s — IR tables in defs + outTag AccAdd densify + maps + iterative `+` (list stays 5; see `self_host/seed/README.md`) |
...
| **GP6n** | `game/xr` | **VR/AR-stöd** — headset present, tracked controllers, stereo cameras; WebXR/OpenXR via host FFI | ✅ subset (swapchain/layer + inputSources + profiles + **XR_EXT_hand_tracking** locate stub → live buffers + Promise/Vulkan/D3D11/rAF) |
...
**Checkpoint GP (nästa):** P6b empty skip-list when leaf self-host <10s; resolve real `xrLocateHandJointsEXT` when loader exposes EXT.  
**Checkpoint GP (landad):** `xrLocateHandJoints` stub path (26 joints → live buffers); `xrSetHandJointBuffer` / clear; `liveBuffers`/`locatePath` status; profiles + joints; inputSources/select/squeeze; layer submit; Promise reject; rAF auto-locate; P6 seed-only + P6b densify gate.  
