...
| **Nästa kod** | Live `http_fetch_async` via Kab TLS (TCP); rustls **används inte** när den vägen finns. Arkivera rustls när Kab är stabil. |
| **Inte nu** | Ny `src/**/*.rs`; radera Rust; sqlIs*-kloner; `async fn*` |
| **Historik** | v0.2–v2.x och master-fetch ligger **nedanför** Våg SH. Språkparitet: [LANGUAGE.md](LANGUAGE.md). SH-detalj: [Våg SH](#våg-sh--self-host-självständig-snabb-stabil-). |

**Innehåll:** [Nolltolerans](#nolltolerans--strikt-arbetsregel) · [Egna fötter](#kabootar-på-egna-fötter--noll-rust) · [FT fart](#våg-ft--fart-alla-tekniker-i-kab) · [Våg SH](#våg-sh--self-host-självständig-snabb-stabil-) · [GP](#våg-gp--game-production-3d-motor-pipeline-) · [SC](#våg-sc--science--ai-ta-över-pythons-roll-) · [Master fetch (historik)](#master-fetch-plan-20262027--historik--parity)
...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 SHA/HMAC/PRF/handshake/GCM/X25519/HTTPS-GET + RSA/ECDSA cert-verify + https URL-parse GET + TLS app-data record + HTTP 200 unwrap + case-insensitive HTTP headers + http_fetch URL/record + 302 Location follow + :443 port + chunked decode + multi TLS app-data records + POST body + JSON POST + PUT/PATCH + HEAD/DELETE + OPTIONS/CONNECT + TRACE + UA/Bearer + http_fetch via Kab TLS unwrap + http_fetch_async-shaped Kab TLS + 302/301/303 follow + 307/308 preserve + 10-hop cap + relative Location + cross-origin cred strip + same-origin keep + protocol-relative Location (`sh23_crypto_sha_eval_smoke`, `sh23_crypto_hmac_eval_smoke`, `sh23_crypto_tls_prf_eval_smoke`, `sh23_crypto_tls_hs_eval_smoke`, `sh23_crypto_tls_gcm_eval_smoke`, `sh23_crypto_x25519_eval_smoke`, `sh23_crypto_tls_https_eval_smoke`, `sh23_crypto_tls_cert_eval_smoke`, `sh23_crypto_tls_ecdsa_eval_smoke`, `sh23_crypto_http_get_eval_smoke`, `sh23_crypto_http_resp_eval_smoke`, `sh23_crypto_http_fetch_eval_smoke`); rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** sluta använda rustls när live `http_fetch_async` går via Kab TLS (arkivera rustls-host när Kab är stabil). |
...
**SH6 = KLAR** mot stoppkriteriet (appar + using). **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** `.kbc`/`.kbcb` disk-eval. **SH20** JSON parse/stringify. **SH21** join+norm+write+read. **SH22 deepen:** `sqlExecOk` INSERT+SELECT eq (`sh22_sql_exec_smoke`). **SH23 SHA/HMAC/PRF/handshake/GCM/X25519/HTTPS-GET + RSA/ECDSA cert-verify + https URL-parse GET + TLS app-data record + HTTP 200 unwrap + http_fetch URL/record + 302 Location follow + :443 port + chunked decode + multi TLS app-data records + POST body + JSON POST + PUT/PATCH + HEAD/DELETE + OPTIONS/CONNECT + TRACE + UA/Bearer + http_fetch via Kab TLS unwrap + http_fetch_async-shaped Kab TLS + 302/301/303 follow + 307/308 preserve + 10-hop cap + relative Location + cross-origin cred strip + same-origin keep + protocol-relative Location i Kab (`sh23_crypto_sha_eval_smoke`, `sh23_crypto_hmac_eval_smoke`, `sh23_crypto_tls_prf_eval_smoke`, `sh23_crypto_tls_hs_eval_smoke`, `sh23_crypto_tls_gcm_eval_smoke`, `sh23_crypto_x25519_eval_smoke`, `sh23_crypto_tls_https_eval_smoke`, `sh23_crypto_tls_cert_eval_smoke`, `sh23_crypto_tls_ecdsa_eval_smoke`, `sh23_crypto_http_get_eval_smoke`, `sh23_crypto_http_resp_eval_smoke`, `sh23_crypto_http_fetch_eval_smoke`). rustls-host kvar. Nästa: sluta använda rustls när live `http_fetch_async` går via Kab TLS** (inte fler flagg-kloner). Inte async fn*. Inte fler sqlIs*-kloner.
