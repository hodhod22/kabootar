...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 SHA-256 FIPS eval (`sh23_crypto_sha_eval_smoke`); HMAC/TLS/rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** HMAC-SHA256-eval, sedan TLS-handskakning, därefter rustls-delete |
...
**SH6 = KLAR** mot stoppkriteriet (appar + using). **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** `.kbc`/`.kbcb` disk-eval. **SH20** JSON parse/stringify. **SH21** join+norm+write+read. **SH22 deepen:** `sqlExecOk` INSERT+SELECT eq (`sh22_sql_exec_smoke`). **SH23 SHA-256 FIPS eval i Kab (`sh23_crypto_sha_eval_smoke`; ToInt32 behåller `0x80000000`). HMAC/TLS/rustls-host kvar. Nästa: HMAC-SHA256-eval, därefter TLS-handskakning och rustls-delete** (inte fler flagg-kloner). Inte async fn*. Inte fler sqlIs*-kloner.
