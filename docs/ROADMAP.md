...
**Aktivt fokus (2026-08):** P6b toolchain — `eLocalMap`/map-only `emitSym` + early `IDENT=` parse; full-leaf budget ~643 s; GP6n EXT locate resolve probe; skip-list stays 5 until <10s. **P/GP0–GP5** + **GP6a–n subset** + **SC0–SC7** + **DX0–DX7** + SIM/DATA/IOT/APP MVP subset landad.
...
| **P6** | **Self-host compile-tid** — tömma H6e skip-list; snabbare parse/emit; incremental `.kbc`; committed `self_host/seed/*.kbc` | ✅ **seed-only**. **P6b** 📋 <10s — toolchain maps/`emitSym` + early `IDENT=` + leaf densify (list stays 5; ~643 s serialize_body; see `self_host/seed/README.md`) |
...
**Checkpoint GP (nästa):** P6b empty skip-list when leaf self-host <10s; call resolved `xrLocateHandJointsEXT` when addr ≠ 0 (replace synth).  
**Checkpoint GP (landad):** loader probe records `xrLocateHandJointsEXT`; `xrLocateHandJoints` stub/synth → live buffers (26 joints); `extResolved`/`locatePath` status; set/clear buffers; rAF auto-locate; P6 seed-only + P6b toolchain maps gate.  
