...
| **SH17** | **JIT i Kabootar** — register-alloc + native-emit som `.kab` (ersätt Cranelift i `src/`). Får börja som subset (i64-loopar). Bred teknik: [Våg FT](#våg-ft--fart-alla-tekniker-i-kab) F7–F10 | Hot loop kompilerad av Kab-JIT; `src/` JIT-moduler raderas när smoke är grön | ✅ subset + **`jitFromOpsOk`** AccAdd-local/`add`/`const`/`load_local`/`store_local`/`ret` → inc+ret `os_mm_call` (`sh17_jit_from_ops_exec_smoke`); i64-loop-subset stängt; **breddad op-legalitet** (`jitOpsIsColdLegal`: stack/globals/control + hela i64-aritm/bit/cmp-hot-setet) + ärlig `jitCanCompile` (non-array/`[]` reject) (`sh17_jit_ops_wide_exec_smoke`); templates + `os_mm_*`; ingen ny Cranelift/RWX i Rust |
