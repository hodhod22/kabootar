...
| **Nästa kod** | SH5: reverse-densify fortsatte — nu **8** filer; `parser_postfix`→`parser_exec` var enda möjliga merge under `bootPolicy("maxBytes")` 64 KiB (`parser_stmt`/`emit_*_body` skulle överskrida). Vidare densify kräver först mindre blad (splitta `emit_expr_body` 72 KiB / `parser_stmt` 57 KiB). SH18: nästlade plain-objekt dedupas i mark-worklist via `gc_obj_id` pointer-identity (gate `sh18_gc_vm_plain_exec_smoke`); `Undefined+1` löst i deep-9. SH17: 4-vägs PIC nu kopplad till fused-dispatchern — `jit_pic_exec` (`jitPicPlan` site-plan + `jitPicSlotsOk` fusibilitets-gate + `jitPicExecOk` slot→`jitFromOpsAnyExecOk`). Nästa steg: PIC i riktig dispatch-loop eller SH18-territorium. Nästa gate: SH28 arkivera `src/` när alla bootstrap/AOT/no-rustc gater passerar. |
...
| **SH5** | **Reverse-densify** — slå ihop tunna shards; compile-DAG **< 80** | SH0 ner; leaf ≤10 s | ✅ platå **8** filer (compile + lexer_scan + parser_exec/parser_stmt + emit_expr_body/stmt_body + serialize_sections + ast_defs) — `emit_exec`/`ownership`→`compile`, `emit_sym`→`ast_defs`, `parser_postfix`→`parser_exec`; gamla paths = `pub import`-facader. **`match`** parse+emit: const/`_`/`Some`/`None`/`Ok`/`Err` + guards + array/objekt. Merge 4 valdes via `bootPolicy("maxBytes")` = 64 KiB: `parser_exec`+`postfix` ≈48 KiB enda möjliga par — `parser_stmt`/`emit_*_body`-merger hade överskridit taket |
