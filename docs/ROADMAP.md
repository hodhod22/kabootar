...
| **SH19** | **Laddare i Kab** — ingen `main.rs` som produkt. `.kab` startar runtime | `kabootar`-binär = Kab-bootstrap eller Kab-AOT | ✅ subset + **`loadEvalKbcRoundtripOk`** persist/read/eval `.kbc` (`sh19_load_file_exec_smoke`); `loadIsKab`/`loadEntry`/`loadIsKbc` + AOT capstone; `loadMainDeleteOk` still false |
...
**SH6 = KLAR** mot stoppkriteriet (appar + using). **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19 deepen:** `loadEvalKbcRoundtripOk` os_write/os_read `.kbc` + Kab-VM (`sh19_load_file_exec_smoke`). **Nästa:** SH19 `.kbcb` open+deserialize (inte dual-bind). Inte async fn*. Inte fler generator-dual-bind. Inte fler GC-flagg-kloner.
