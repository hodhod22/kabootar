...
**Nästa:** `using`/`with` är self-host parse+emit + rust-VM; Kab-VM **anropar** `close?.()` men sidoeffekten syns inte i anroparen (lastThis/`store_global`/host-`CALL` från klassmetod). Ny trampolin/writeback/native är ny arkitektur. `let m = super.tag` på **`Child<T>`** fallerar i emit (`methods` är null på specialiserad klass); icke-generisk `Child` är proven. Ny klass-tabell-lookup är ny arkitektur. `import("math")` / `dynamic_import` är self-host+rust-VM; guest-import under `KAB_VM_EXEC_ACTIVE` går via host `eval_program` (inte Kab-VM-proven). `async fn` är ny arkitektur (Kab-VM slänger `fn_async`; rust `schedule_bytecode_async`). `for await` behöver en native som returnerar uppdaterad iterator (inte ny native nu). `yield` är generator-arkitektur. 1k+ loop är SH17/JIT. Inte höja text-`maxKbc`. Inte `noll_*`. Inte SH17–SH28. Inte rust-opcode för `is` eller rust-compile av default/rest-params.
