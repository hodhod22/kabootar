...
| **F7** | **Baseline JIT i Kab** — template per opcode → maskinkod (SH17 start) | `jit_stats` hits från Kab-JIT, inte Cranelift | ✅ subset smoke-complete: templates + `jitMapIncRetOk` + `jitPageSize` + `jitPagesFor` + `jitExecOk` + `jitMmapOk` + `os_mm_store`/`os_mm_call` loop8/loopN/arith-imm/bit-ops/shifts/unary/eq/ne/lt/gt/le/ge mmap/exec dual-bind; host exec deepen (ingen ny Rust-JIT) |
| **F8** | **Optimizing JIT** — SSA, inlining, LICM, GVN, deopt; **typspecialisering** från IC-feedback (F20); **TCO opt-in** (F24) | i64-loop nära native minus skatt | ✅ subset: `jitCanInline` + `jitLicmOk` + `jitGvnOk` + `jitDeoptOk` + `jitSsaOk`; native-loop / spec / TCO deepen |
...
| **F14** | **I/O-stack** — async FS/net utan extra copy | SH21/SH24; e2e vs Node | ✅ subset: async FS + HTTP host wraps inkl. `httpServe`/`httpServeOnce` (ingen live-fetch/serve-loop i smoke); radera host-HTTP deepen |
...
| **F21** | **Lazy AOT** — AOT bara boot-graf (första ~100 ms / import-DAG); resten baseline-JIT + F8 | HTTP kallstart-budget (målordning: mät → kapa boot, inte 50 ms-löfte först) | ✅ subset: `aotLazyOk` (≤100 ms) + `aotLazyJitOk` (remainder JIT) dual-bind `aotPgoOk`; live boot-DAG deepen |
| **F22** | **TLAB** — per-tråd nursery; promote till delad old när TLAB full | workers (F13) allokerar utan mutex per `new`; pause-histogram oförändrat | ✅ subset: `tlabOk` (≥2 workers) + `tlabPromoteOk` (full → old) dual-bind `gcHostDeleteOk`; pause-histogram deepen |
...
| **SH17** | **JIT i Kabootar** — register-alloc + native-emit som `.kab` (ersätt Cranelift i `src/`). Får börja som subset (i64-loopar). Bred teknik: [Våg FT](#våg-ft--fart-alla-tekniker-i-kab) F7–F10 | Hot loop kompilerad av Kab-JIT; `src/` JIT-moduler raderas när smoke är grön | ✅ subset smoke-complete: templates + map/page + `jitMmapOk` + `os_mm_store`/`os_mm_call` loop8/loopN/arith-imm/bit-ops/shifts/unary/eq/ne/lt/gt/le/ge dual-bind; exec deepen utan ny Cranelift/RWX i Rust |
| **SH18** | **GC i Kabootar** — nursery/sweep som `.kab` (ersätt Rust-GC) | Frame-budget smoke utan `src/` GC; sedan radera | ✅ subset smoke-complete: bump + promote + sweep + barrier + `gcMarkStep` + `gc_load`→`gc_concurrent_stress`→`gc_chain`→`gc_capstone` + `gcHostDeleteOk` dual-bind; radera host-GC deepen |
