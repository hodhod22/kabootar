...
| **Nästa kod** | SH5: vidare stegvis reverse-densify — nu **9** filer; nästa kandidater är `parser_stmt`/`parser_postfix`/`emit_*_body` mot compile-hus när leaf ≤10 s håller, alltid en merge i taget + probe. SH18: `Undefined+1`-regressionen och test-racen är lösta — nästa GC-gap är nästlade plain-objekt/identitets-dedup (objekt utan heap-slot kan inte dedupas i mark-worklisten utan objekt-identity) eller SH17-JIT om fart prioriteras. Nästa gate: SH28 arkivera `src/` när alla bootstrap/AOT/no-rustc gater passerar. |
...
| **SH5** | **Reverse-densify** — slå ihop tunna shards; compile-DAG **< 80** | SH0 ner; leaf ≤10 s | ✅ platå **9** filer (compile + lexer_scan + parser_exec/postfix/stmt + emit_expr_body/stmt_body + serialize_sections + ast_defs) — `emit_exec`/`ownership`→`compile`, `emit_sym`→`ast_defs`; gamla paths = `pub import`-facader. **`match`** parse+emit: const/`_`/`Some`/`None`/`Ok`/`Err` + guards + array/objekt. Inte merge `parser_stmt`/`postfix`/`emit_*_body` förrän leaf ≤10 s / ~550 rader |
