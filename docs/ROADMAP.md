...
| **F7** | **Baseline JIT i Kab** — template per opcode → maskinkod (SH17 start) | `jit_stats` hits från Kab-JIT, inte Cranelift | ✅ subset: templates + `jitMapIncRetOk` + `jitPageSize` + `jitPagesFor` + `jitExecOk` + `jitMmapOk` mmap/exec dual-bind; host exec deepen (ingen ny Rust-JIT) |
...
| **F11** | **Nursery GC i Kab** (SH18) — bump, promote, frame-budget | 60 FPS utan GC-spike i idle | ✅ subset: bump + promote + sweep + `gcWriteBarrier` + `gcMarkStep` + `gcHostDeleteOk` dual-bind; concurrent mark deepen |
...
| **SH17** | **JIT i Kabootar** — register-alloc + native-emit som `.kab` (ersätt Cranelift i `src/`). Får börja som subset (i64-loopar). Bred teknik: [Våg FT](#våg-ft--fart-alla-tekniker-i-kab) F7–F10 | Hot loop kompilerad av Kab-JIT; `src/` JIT-moduler raderas när smoke är grön | ✅ subset: templates + map/page + `jitMmapOk` mmap/exec dual-bind; exec deepen utan ny Cranelift/RWX i Rust |
| **SH18** | **GC i Kabootar** — nursery/sweep som `.kab` (ersätt Rust-GC) | Frame-budget smoke utan `src/` GC; sedan radera | ✅ subset: bump + promote + sweep + barrier + `gcMarkStep` + `gcHostDeleteOk` dual-bind; radera host-GC deepen |
| **SH19** | **Laddare i Kab** — ingen `main.rs` som produkt. `.kab` startar runtime | `kabootar`-binär = Kab-bootstrap eller Kab-AOT | ✅ subset: `loadIsKab` / `loadEntry` / `loadIsKbc` / `loadImageName` + `loadMainDeleteOk` dual-bind; radera `main.rs` deepen |
