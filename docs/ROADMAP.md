...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 Kab TLS unwrap + loopback hex-TCP + query Location (`sh23_crypto_http_fetch_eval_smoke`); rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** se [Just nu](#just-nu). |
