...
**Aktivt fokus (2026-08):** P6b `serialize_body` shallow AccAdd densify + fas-remeasure; GP6n live hand-joint buffers (`xrSetHandJointBuffer`); skip-list stays 5 until <10s. **P/GP0–GP5** + **GP6a–n subset** + **SC0–SC7** + **DX0–DX7** + SIM/DATA/IOT/APP MVP subset landad.
...
| **P6** | **Self-host compile-tid** — tömma H6e skip-list; snabbare parse/emit; incremental `.kbc`; committed `self_host/seed/*.kbc` | ✅ **seed-only**. **P6b** 📋 <10s — densify (membership + shallow AccAdd appends) + maps + iterative `+` + op-count jump patches (list stays 5; see `self_host/seed/README.md`) |
...
| **GP6n** | `game/xr` | **VR/AR-stöd** — headset present, tracked controllers, stereo cameras; WebXR/OpenXR via host FFI | ✅ subset (swapchain/layer + inputSources + profiles + **XR_EXT_hand_tracking** status/stub + **live joint buffers** + Promise/Vulkan/D3D11/rAF) |
...
**Checkpoint GP (nästa):** P6b empty skip-list when leaf self-host <10s; wire live buffers from real OpenXR `XR_EXT_hand_tracking` locations.  
**Checkpoint GP (landad):** live hand joint buffers (`xrSetHandJointBuffer` / `xrClearHandJointBuffer` / `liveBuffers`); `XR_EXT_hand_tracking` status (`emulated`/`openxr-stub`/`openxr-ext`); profiles + joints; inputSources/select/squeeze; layer submit; Promise reject; rAF; P6 seed-only + P6b densify gate.  
