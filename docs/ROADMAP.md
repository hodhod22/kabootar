...
| **SH6** | **Kab-VM som produktväg** — inte evig bootstrap | `KABOOTAR_VM=kab-only` default-smoke; `vm_*` **< 40** | ✅ subset: **kab-only default**; **`.kbcb` run** via mmap; **`in`**; **`await`**; **`array_slice_from`**; **`new_instance_from_array`**; self-host **`let [a, ...rest]`** / **`let { a, ...rest }`** / **`let { x: [a, b] }`** / **`[1, ...xs]`** / **`{ ...obj }`** / **`{ a }`** / **`{ foo() {} }`** / **`{ [k]: v }`** / **`is`/`is not`** / **`fn f(a, b = 3)`** / **`fn f(a, ...xs)`** / **`(a, b = 3) =>`** / **`(a, ...xs) =>`** / **`class` method `fn add(a, b = 3)`** / **`fn rest(a, ...xs)`** / **`{ add(a, b = 3) {} }`** / **`{ rest(a, ...xs) {} }`** / **`trait` default `fn add(a, b = 3)`** / **`fn rest(a, ...xs)`** / **`o?.x`** / **`xs?.[0]`** / **`f?.()`** / **`delete o.z`** / **`switch`** / **`fallthrough`** / **`do while`** / **`xs[0] +=`** / **`step()?`/`bad()?`** / **`? :`** / **`import.meta`** / **`` `n=${n}` ``** / **`||=` `&&=` `??=`** / **`??`** / **`for let i = 0`** / **`for x of`** / **`for k in`** / **`match 1 { 1 => 2, _ => 0 }`** / **`match [x, y]`** / **`match { p, q }`** / **`if let Some(x)`** / **`while let Ok(v)`** / **`match 1..=5`** / **`n @ 1..=5`** / **`1 | 2 | 3`** / **`..5`** / **`5..`** / **`[h, ...t]`** / **`{ k, ...s }`** / **`[h, ...mid, last]`** / **`n @ 1..=5 if n != 3`** / **`Color.Red`** / **`Msg.Move(p)`** / **`xs @ [p, q]`** / **`wrap @ { k, ...s }`** / **`{ k: n @ 1..=5 }`** / **`[n @ 1, ...r]`** / **`Ok(n @ 1..=5)`** / **`Some(n @ 1..=5)`** / **`if let 1 | 2`** / **`while let 1 | 2`** / **`(1 | 2)`** / **`Option.Some(n)`** / **`Option.Some("x")`** / **`Option<Number>.None`** / **`1.0..=2.0`** / **`Result.Ok(n)`** / **`Result<Number, String>.Err`** / **`if let n @ Some(x)`** / **`n @ 1 | 2`** / **`v @ Msg.Move(x)`** / **`struct Box<T>`** / **`Box$String`** / **`is(obj, "Class")`** / **`pass`/`assert`/`not`** / **`raise`** / **`o.x +=`** / **`o.a.b +=`** / **`o.items[0] +=`** / **`o.items[0][0] +=`** / **`xs[0].x +=`** / **`xs[0][0].x +=`** / **`o.items[0][0].x +=`** / **`xs[0][0] +=`** / **`xs[0][0][0] +=`**; **kbcb v2** + **`op.tag`→leaf** + **Uint8Array/SAB** + **file-mmap**; **100** loop / **200** unrolled; `object_array_push`; 1k+ nested-interpreter-bundet; text-`maxKbc` 256KiB; **`maxKbcb` 8MiB**; **`get_member`+`call`**; **`get_super_method`** |
...
**Nästa:** `using`/`with` är self-host parse+emit + rust-VM; Kab-VM **anropar** `close?.()` men sidoeffekten syns inte i anroparen (lastThis/`store_global`/host-`CALL` från klassmetod). Ny trampolin/writeback/native är ny arkitektur. `import("math")` / `dynamic_import` är self-host+rust-VM; guest-import under `KAB_VM_EXEC_ACTIVE` går via host `eval_program` (inte Kab-VM-proven). `async fn` är ny arkitektur (Kab-VM slänger `fn_async`; rust `schedule_bytecode_async`). `for await` behöver en native som returnerar uppdaterad iterator (inte ny native nu). `yield` är generator-arkitektur. 1k+ loop är SH17/JIT. Inte höja text-`maxKbc`. Inte `noll_*`. Inte SH17–SH28. Inte rust-opcode för `is` eller rust-compile av default/rest-params.
