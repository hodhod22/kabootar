...
**Aktivt fokus (2026-08):** P6b emit If early-dispatch + AccAdd recurse (remeasure `serialize_body`); GP6n WebXR `inputSources`/select/squeeze subset. Skip-list stays 5 until <10s. **P/GP0–GP5** + **GP6a–n subset** + **SC0–SC7** + **DX0–DX7** + SIM/DATA/IOT/APP MVP subset landad.
...
| **P6** | **Self-host compile-tid** — tömma H6e skip-list; snabbare parse/emit; incremental `.kbc`; committed `self_host/seed/*.kbc` | ✅ **seed-only product path** (5 leaves + fingerprint gates). **P6b** empty list 📋 när budget <10s — leaf densify + AccAdd recurse + early `emitIfStmt` (list stays 5; see `self_host/seed/README.md`) |
...
| **GP6n** | `game/xr` | **VR/AR-stöd** — headset present, tracked controllers, stereo cameras; WebXR/OpenXR via host FFI | ✅ subset (wgpu → swapchain + layer submit + **inputSources**/select/squeeze/grip/targetRay + Promise.then reject + Vulkan/D3D11/await/create/end-frame) |
...
**Checkpoint GP (nästa):** P6b empty skip-list when leaf self-host <10s; richer XR profiles/hand tracking.  
**Checkpoint GP (landad):** WebXR `inputSources` + select/squeeze + grip/targetRay poses (stub); wgpu/Vulkan image handles through layer submit; `xrSessionThen` rejection; Vulkan/D3D11 + await rAF; P6 seed-only + P6b gate.  
