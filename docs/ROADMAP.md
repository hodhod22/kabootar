...
**Aktivt fokus (2026-08):** **Våg SH** — SH2/SH13 sess reuse; SH8 tiny parse CI; SH7b; SH12–SH15 subset; SH9 `jit_add_loop`. Full tiny `compile()` ignored i debug. Ny produktlogik i `.kab`, inte Rust. **L/O/T/J/S** ✅ subset; **P0–P9** + **GP/SC/DX** MVP subset landad.
...
| **SH8** | **Användbarhets-gate** — tiny parse via compiler-image i default CI; full `compile_source_self_host("return 1")` ignored i debug tills pipelinen inte hänger | `sh8_tiny_parse_via_compiler_image`; `p10_self_host_tiny_source_profile` ignored | ✅ parse-gate (`parseStmt` dispatch + runaway); full `compile()` still ignored |
| **SH9** | **Compiler på host-JIT** — när toolchain körs på host-VM: Cranelift (P13) på typed i64-hjälpare i emit/serialize (AccAdd-loopar, index-loopar). Inte “JIT:a parsern i Kab-VM” | `jit_stats` hits > 0 under `compile()` av en medium-fil; ingen ny produktlogik i Rust | ✅ subset `sh9_host_jit_add_loop` (`jit_add_loop`); full emit-loop hits deepen |
