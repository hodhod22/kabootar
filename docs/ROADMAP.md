...
**Aktivt fokus (2026-08):** P6b empty skip-list when self-host leaf compile is CI-fast; full OpenXR instance/session + Promise XRSession.rAF. **P/GP0–GP5** + **GP6a–n subset** + **SC0–SC7** + **DX0–DX7** + SIM/DATA/IOT/APP MVP subset landad.
...
| **GP6n** | `game/xr` | **VR/AR-stöd** — headset present, tracked controllers, stereo cameras; WebXR/OpenXR via host FFI | ✅ subset (live session + xrEndFrame FFI + XRSession.rAF + compositor/HMD) |
...
**Checkpoint GP (nästa):** full xrCreateInstance/xrCreateSession with real HMD; browser Promise-resolved XRSession.rAF; P6b empty skip-list when leaf self-host <10s.  
**Checkpoint GP (landad):** live OpenXR session + real xrEndFrame FFI (`stub-trampoline`/`direct`); wasm XRSession.rAF when session granted (`webxr-session-raf`); loader end-frame + immersive rAF; P6 seed-only + P6b gate.  
