...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 Kab TLS application-data/AES-GCM unwrap över loopback hex-TCP + query Location (`sh23_crypto_http_fetch_eval_smoke`); rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** byt `http_fetch_async` från rustls till Kab-TLS när `tcp_read` kan bära råa TLS-bytes. |
