...
| | |
|--|--|
| **Våg / steg** | **SH23** — krypto + TLS 1.2 **och TLS 1.3** i Kab (`cryptoHostDeleteOk` driven by `crypto_tls13_all.kab`; rustls delete gate unlåst när all SH23-smoke passerar) |
| **Nästa kod** | rustls-host är kvar (`cryptoHostDeleteOk` false). Kab TLS 1.3 SAN iPAddress length 11–15 rejected (`sh23_crypto_tls13_peer_n11..n15_eval_smoke`); parametriserad längdsvepning 0–20 (`sh23_crypto_tls13_peer_nlen_sweep_eval_smoke`); leaf-cert `validity` notBefore/notAfter (`sh23_crypto_tls13_peer_dates_eval_smoke`); chain-count, chain-all, serial, sigAlg, pin, sammansatt trust-gate, X.509-struktur, AIA/OCSP, full handshake, high-level client (`crypto_tls13_client.kab`/`crypto_tls13_client_get.kab`/`crypto_tls13_client_close.kab`/`crypto_tls13_client_fetch.kab`/`crypto_tls13_client_reconnect.kab`/`crypto_tls13_client_config.kab`/`crypto_tls13_client_post.kab`), scheme-match (`crypto_tls13_peer_scheme.kab`) och no-rustls loopback (`crypto_tls13_client_loop.kab`); `httpFetch` kan nu nå rustls-peer via Kab-klient (`crypto_tls13_client_fetch.kab`); TLS 1.3 applikationsdata-GET (`sh23_crypto_tls13_peer_appdata_eval_smoke`); generaliserad SAN-validator (`crypto_tls13_peer_san_gen`) + konsoliderad ext-scanner (`crypto_tls13_hs_opt`). Cheap smokes importerar inte crypto. Nästa gate: ersätta rustls-hosten (inte radera rustls förrän stabil). |
