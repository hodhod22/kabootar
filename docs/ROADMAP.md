...
**Nu:** språk i self-host (`match` enum + `if let`/`while let`) → SH5 platå (DAG 12) → SH16 redan stängd för appar → Kab-VM/JIT/GC deepen → **fart i Kab (Våg FT)** → stdlib–CLI i `.kab` (SH20–27) → radera `src/` (SH28). Planer: [egna fötter](#kabootar-på-egna-fötter--noll-rust) · [fart](#våg-ft--fart-alla-tekniker-i-kab).
...
| **F7** | **Baseline JIT i Kab** — template per opcode → maskinkod (SH17 start) | `jit_stats` hits från Kab-JIT, inte Cranelift | ✅ subset smoke-complete: templates + `jitMapIncRetOk` + `jitPageSize` + `jitPagesFor` + `jitExecOk` + `jitMmapOk` + `os_mm_store`/`os_mm_call` loop8/loopN mmap/exec dual-bind; host exec deepen (ingen ny Rust-JIT) |
...
| **F11** | **Nursery GC i Kab** (SH18) — bump, promote, frame-budget | 60 FPS utan GC-spike i idle | ✅ subset: bump + promote + sweep + barrier + `gcMarkStep` + `gc_load`→`gc_chain`→`gc_capstone` + concurrent mark/stress + `gcHostDeleteOk` dual-bind; radera host-GC deepen |
...
| **L6** | **Self-host `match`** — parse+emit i `parser_postfix` / `emit_expr_body` så appar (SH16, ingen Rust-emit) kan kompilera `match` | ✅ subset: const/`_`/var/`Some`/`None`/`Ok`/`Err` + guards + **array/objekt** + **enum-mönster** (`Type.Variant` / fält) + **`if let`/`while let`** (socker → `match`) + Kab-VM `jump_unless_*`/`match_fail`/`unwrap_*`/`jump_unless_enum_variant`/`unpack_enum_fields`. Kvar: enum-sektion i text-`.kbc` (runtime `Color.Red` utan host-enum-register) |

**Checkpoint L1–L5:** `cargo test --test v228_language bytecode_` + `cargo test --test ownership_manual` + `cargo test --test s2_compile_cli`  
**Checkpoint L6:** `cargo test --test self_host self_host_match_const_wildcard_compile_run` + `self_host_match_array_object_compile_run` + `self_host_match_const_kab_only` + `self_host_if_let_some_compile_run` + `self_host_while_let_ok_compile_run` + `self_host_match_enum_pattern_ir`
...
| **S4** | **Språkparitet i produktkompilatorn** — `match` parse+emit (L6); inte bara host-Rust | ✅ subset (`if let`/`while let` + enum-mönster-IR; text-`.kbc` enum-register kvar) |
...
| **SH17** | **JIT i Kabootar** — register-alloc + native-emit som `.kab` (ersätt Cranelift i `src/`). Får börja som subset (i64-loopar). Bred teknik: [Våg FT](#våg-ft--fart-alla-tekniker-i-kab) F7–F10 | Hot loop kompilerad av Kab-JIT; `src/` JIT-moduler raderas när smoke är grön | ✅ subset smoke-complete: templates + map/page + `jitMmapOk` + `os_mm_store`/`os_mm_call` loop8/loopN dual-bind; exec deepen utan ny Cranelift/RWX i Rust |
...
| **SH20** | **Stdlib i Kab** — sträng, array, objekt, math, JSON, datum, regex, collections som `.kab` (ersätt `src/runtime/stdlib`) | Smoke utan Rust-natives för kärn-API; radera motsvarande `.rs` | ✅ subset: `stdAdd`/`stdLen`/`stdHas` + `stdJsonIsNull` + `stdDateEpochOk` + `stdReHit`; radera natives deepen |
| **SH21** | **OS/FS/process i Kab** — `import "os"` policy + I/O i `.kab` (kOS). Rust `runtime/os` skuld | App läser/skriver filer via Kab-OS; host-FS bara kapabilitet tills drivrutin är Kab | ✅ subset: `kabOsIsVfs` / caps + `kabOsIsFile` + `kabOsArgvOk`; `kos/vfs` + radera `runtime/os` deepen |
| **SH22** | **SQL i Kab** — query/storage i `.kab` (ersätt `src/sql`) | `sql()` smoke utan Rust-motor | ✅ subset: `sqlIsSelect` / `sqlScalarOne` + `sqlIsWhere` + `sqlStoreOk`; radera `src/sql` deepen |
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | ✅ subset: `cryptoIsHttps` / pin/trust + `cryptoTls12Ok` + `cryptoRootPem`; rustls-delete deepen |
| **SH24** | **HTTP i Kab** — server/fetch ovanpå SH21/SH23 | `http_fetch_async` / `http_serve` utan `src/runtime/http.rs` | ✅ subset: host wraps + `httpIsPost` + `httpIsJson`; radera `runtime/http.rs` deepen |
| **SH25** | **CLI, REPL, test-runner i Kab** — `kabootar run/compile/test` | CI kan köra `.kab`-gates utan `src/cli` | ✅ subset: run/repl/test + `cliIsCompile` + `cliIsFmt`; radera `src/cli` + kabtest KT8 deepen |
| **SH26** | **Science/GPU-API i Kab** — kernels och nd i `.kab`; native GPU bara syscall | Science-smoke på Kab-VM/JIT | ✅ subset: `sciAdd`/`sciMul`/`sciGpuOff` + `sciNdLenOk` + `sciFftPow2`; GPU kernel deepen |
| **SH27** | **Browser/DOM/game i Kab** — kbrowser + canvas/game-loop i `.kab` | UI-smoke utan `src/runtime/browser*` produktlogik | ✅ subset: `uiIsDiv` / `uiTickMs` + `uiIsCanvas` + `uiFpsOk`; kbrowser deepen |
| **SH28** | **Radera produkt-Rust** — tom `src/` för runtime; inget rustc för att köra Kabootar | `src/**/*.rs` produkt = 0; dokumenterad bootstrap-image från Kab | ✅ subset: `nollSrcGoal=0` + `nollAotReady=false` + `nollKeepSrc`; **radera inte `src/`** förrän AOT |
...
**Nästa:** språk i self-host — enum-register i text-`.kbc` (kör `Color.Red`); SH20–SH28 delete-gate deepen (alla gates fortfarande `false`); SH17 JIT exec round-trip deepen. Se [Kabootar på egna fötter](#kabootar-på-egna-fötter--noll-rust). **Dok:** ROADMAP + `self_host/README.md` efter varje pass.
