...
**Princip:** mät (F0) → gör hot path billig i VM (F1–F6) → JIT/AOT (F7–F10) → minne (F11–F12) → parallell/I/O/GPU (F13–F16) → liga-gates (F17) → **app-CI + same-room zero-copy + TLAB + lazy AOT + TCO (F18–F24)**. Ingen ny Cranelift/IC/GC-feature i `src/`.
...
| **JIT (SH17)** | Baseline (template/copy-and-patch) → optimizing (SSA, GVN, LICM, inlining); linear-scan eller graph coloring regalloc; deopt + OSR; on-stack replacement; type feedback från IC; **typspecialisering** av heta fn (i64/f64); **TCO opt-in**; i64-loopar först sedan f64/SIMD |
| **AOT** | Fingerprint → native image (Kab-emit, inte LLVM-as-product); PGO; **lazy AOT** = boot-grafen AOT, resten JIT |
| **GC (SH18)** | Nursery bump + promote; **TLAB / per-tråd nursery**; incremental/concurrent mark; frame-budget för 60 FPS; write barriers i Kab; `@manual` utan checks i release |
| **Minne** | Region/bump per compile-session (SH13 deepen); object pooling för tokens/ops; zero-copy buffers till GPU/FS; **mmap/`os_mm` för stora homogena arrayer** |
| **Parallell** | Workers/job-system; compiler-blad parallellt (SH7); data-parallel `job_map` på unboxed arrays |
| **I/O** | io_uring/IOCP bakom Kab-`os` (SH21); HTTP without JSON hop (P17/F15); SQL same-process; **SQL-rad → HTTP-svar som view (F19)** |
| **SIMD/GPU** | vec/matmul/FFT som Kab-API + SIMD-JIT eller GPU-kernel; inga per-element boxed loops |
| **Profil** | `KABOOTAR_P10_PROFILE`, op-histogram, `jit_stats`, alloc/frame; PGO tillbaka till JIT; **tre kanoniska appar i CI (F18)** |
...
| **F2** | **IC + shapes** — GetMember/LoadGlobal/CALL monomorf → **PIC ≤4** → megamorf-fallback (V8-modell; redan P12b 2-vägs) | hit-rate > 90 % på typisk app; PIC får inte växa obegränsat | ✅ subset: `icIsMono` + `icIsPoly` + `icIsMega` + `icHitOk` (≥90); GetMember IC deepen till 4-vägs PIC i Kab-JIT |
...
| **F7** | **Baseline JIT i Kab** — template per opcode → maskinkod (SH17 start) | `jit_stats` hits från Kab-JIT, inte Cranelift | ✅ subset smoke-complete: templates + `jitMapIncRetOk` + `jitPageSize` + `jitPagesFor` + `jitExecOk` + `jitMmapOk` + `os_mm_store`/`os_mm_call` loop8/loopN/arith-imm/bit-ops/shifts/unary/eq/ne/lt/gt/le/ge/test/je/jne/jmp/jl/jle/jg/jge/nop mmap/exec dual-bind; host exec deepen (ingen ny Rust-JIT) |
| **F8** | **Optimizing JIT** — SSA, inlining, LICM, GVN, deopt; **typspecialisering** från IC-feedback (F20); **TCO opt-in** (F24) | i64-loop nära native minus skatt | ✅ subset: `jitCanInline` + `jitLicmOk` + `jitGvnOk` + `jitDeoptOk` + `jitSsaOk` + `jitNativeLoopOk` + `jitNativeLoopGvnOk` + `jitNativeLoopDeoptOk` + `jitNativeLoopSpecOk` + `jitNativeLoopTcoOk` + `jitNativeLoopTcoSelfOk` loopN mmap/exec dual-bind; F9 regalloc deepen |
| **F9** | **Regalloc + SIMD i JIT** — linear-scan; later SIMD-unbox | nd-add/dot utan boxed loop | ✅ subset: `jitScanGprs` + `jitColorOk` + `jitSimdOk` + `jitNativeLoopGprOk` + `jitNativeLoopColOk` + `jitNativeLoopSimdOk` + `jitNativeLoopNdAddOk` + `jitNativeLoopNdDotOk` loopN mmap/exec dual-bind; F10 AOT deepen |
| **F10** | **AOT + PGO** — warmed image; profilstyrd JIT/AOT; **lazy AOT** (F21) | kallstart + steady-state gates | ✅ subset: x64/arm64 ship/ret-op/nop-op (load+verify)/zero-op (load+verify)/one-op (load+verify)/add-op (load+verify)/sub-op (load+verify)/mul-op (load+verify)/div-op (load+verify)/and-op (load+verify)/or-op (load+verify)/shl-op (load+verify)/shr-op (load+verify)/not-op (load+verify)/xor-op (load+verify)/neg-op (load+verify)/cmp-op (load+verify)/test-op (load+verify)/je-op (load+verify)/jne-op (load+verify)/jl-op (load+verify)/jle-op (load+verify)/jg-op (load+verify)/jge-op (load+verify)/text+layout+native (one+add+sub+mul+div+and+or+shl+shr+not+xor+neg+cmp+test+je+jne+jl+jle+jg+jge+nop+ret)/rodata/data plus write/read bind; `aotLazyNativeLoopOk` boot≤100 ms + remainder JIT + nd-dot mmap/exec dual-bind; `aotLazyBootDagOk` live import-DAG (boot≤8, remainder>0) dual-bind; `aotLazyBootDagNamedOk` named boot entry dual-bind; `aotLazyBootDagEdgeOk` import-edge dual-bind; `aotLazyBootDagAcyclicOk` reverse-edge reject dual-bind; F11 nursery deepen |
| **F11** | **Nursery GC i Kab** (SH18) — bump, promote, frame-budget; **TLAB** (F22) när workers (F13) är riktiga | 60 FPS utan GC-spike i idle; alloc utan globalt lock per objekt | ✅ subset: bump + promote + sweep + barrier + `gcMarkStep` + `gc_load`→`gc_chain`→`gc_capstone` + concurrent mark/stress + `gcHostDeleteOk` dual-bind + `gcTlabNurseryOk` + `gcTlabPauseOk` + `gcTlabHostDropOk`; F12 `@manual` `escManualReleaseOk` |
| **F12** | **Escape analysis + `@manual` release** — stackalloc; noll checks | use-after-move bara debug | ✅ subset: `kab/esc` `escFitsFrame` + `escManualReleaseOk`; F13 `workParOk` |
| **F13** | **Parallell compile + workers** | SH7/P8 deepen i Kab | ✅ SH7 subset + `workParOk` (≥2, dual-bind TLAB + `@manual` release); F14 `httpWorkDropOk` |
| **F14** | **I/O-stack** — async FS/net utan extra copy | SH21/SH24; e2e vs Node | ✅ subset: async FS + HTTP host wraps inkl. `httpServe`/`httpServeOnce` (ingen live-fetch/serve-loop i smoke) + `httpWorkDropOk`; F15 same-room deepen |
| **F15** | **Same-room webb** — ingen JSON-varv UI↔SQL; zero-copy rad→svar är **F19** | P17 deepen i Kab | ✅ subset |
...
| **F18** | **App-CI** — tre kanoniska `.kab`-appar (same-room HTTP, 60 FPS game-loop, nd/AI-kernel) mot budget; inte bara `league_add_loop` | PR-gate: ingen tyst regression på de tre; mikro får inte vinna mot app | ✅ subset: `appCiHttpOk` (≤100 ms) + `appCiFpsOk` (≥60) + `appCiNdOk` (≤50 ms); live-app deepen (inte ny `src/`-profiler) |
| **F19** | **Zero-copy same-room** — `sql()`-rad / nd-view → `http_request`-kropp utan stringify och utan extra `Value`-kopia | e2e: N rader lista utan JSON-hop (P17b deepen); alloc/request histogram | ✅ subset: `zcSqlViewOk` + `zcNdViewOk` (0 extra copies); live buffer/histogram deepen |
| **F20** | **Typspecialisering** — JIT klonar het fn när IC alltid ser i64 eller f64; deopt vid ny typ | type-check borta i specialiserad loop; samma i64-gate som F8 | ✅ subset: `jitSpecOk` (`i64`) + `jitSpecF64Ok` (`f64`) + `jitSpecDeoptOk` (ny typ) dual-bind `icIsMono`; native clone deepen |
| **F21** | **Lazy AOT** — AOT bara boot-graf (första ~100 ms / import-DAG); resten baseline-JIT + F8 | HTTP kallstart-budget (målordning: mät → kapa boot, inte 50 ms-löfte först) | ✅ subset: `aotLazyOk` (≤100 ms) + `aotLazyJitOk` (remainder JIT) dual-bind `aotPgoOk` + `aotLazyBootDagOk` + `aotLazyBootDagNamedOk` + `aotLazyBootDagEdgeOk` + `aotLazyBootDagAcyclicOk`; F11 nursery deepen |
| **F22** | **TLAB** — per-tråd nursery; promote till delad old när TLAB full | workers (F13) allokerar utan mutex per `new`; pause-histogram oförändrat | ✅ subset: `tlabOk` (≥2 workers) + `tlabPromoteOk` (full → old) dual-bind `gcHostDeleteOk` + `gcTlabNurseryOk` + `gcTlabPauseOk` + `gcTlabHostDropOk`; F12 `@manual` `escManualReleaseOk` |
| **F23** | **Mmap-bulk** — stora `array_f64`/`Uint8Array` via `os_mm` (fil eller anonym); samma pekare till GPU | science/ljud/bild utan extra copy; **inte** “större än RAM som default-heap” | ✅ subset: `mmF64Ok` (`f64` stride 8) + `mmU8Ok` (`u8` stride 1); GPU-pekare deepen |
| **F24** | **TCO opt-in** — svansanrop återanvänder frame (`@tail` eller känd self-recursion i JIT) | djup rekursion utan stack-växt på markerade fn; stackspår bevaras default | ✅ subset: `jitTcoOk` (`@tail`) + `jitTcoSelfOk` (self-recursion) dual-bind `jitSsaOk`; frame-reuse deepen |

**Prisvärd ordning (F18–F24):** F18 (mät appar) → F19 (unique webb-vinst) → F23 (billig, `os_mm` finns) → F22 när F13 är verklig → F20/F24 i Kab-JIT (F8) → F21 när AOT-image faktiskt startar appar. **Inte nu:** PIC med obegränsat många hål (redan F2 max 4); mmap-paging som ersätter GC; TCO på varje anrop; typspec innan IC träffar; blockera varje commit på tre fulla appar innan harnessen finns.

**Koppling till P-vågen:** P0–P18 dokumenterar *vad* som mättes i host-VM. **F0–F24 är samma tekniker, skrivna i Kab**, så de överlever SH28. Att jaga mer Cranelift i `src/bytecode/jit.rs` är **fel riktning**.
...
| **T5** | Self-host: trait/`where` i `self_host/parser` + `emit` | ✅ subset (+ `type Item;` → `interface_assoc_types`; **`type Item = Number`** på klass → `class_assoc_types`; default-metod inject; **`Thing().id()`** / **`id() { return 42 }`** / **`Show<T> default`** / **`Show<T> default override`** → `iface_method_default`; class `fn id()` override; **`where T: Trait`** på generiska fn, metoder och klasser; **`trait Show<T>`** → `interface_type_params` + `implements Show$Number`) |
...
| **R4** | Self-host: `struct`/`self` i parser+emit | ✅ subset (+ **`&self` / `&mut self`** parse + host compile-run; **`struct Box<T>`** specialisering + fält; **`struct WBox<T> where T: Show`**) |
...
| **SH6** | **Kab-VM som produktväg** — inte evig bootstrap | `KABOOTAR_VM=kab-only` default-smoke; `vm_*` **< 40** | ✅ subset: **kab-only default**; **`.kbcb` run** via mmap; **`in`**; **`await`**; **`array_slice_from`**; **`new_instance_from_array`**; self-host **`let [a, ...rest]`** / **`let { a, ...rest }`** / **`let { x: [a, b] }`** / **`[1, ...xs]`** / **`{ ...obj }`** / **`{ a }`** / **`{ foo() {} }`** / **`{ [k]: v }`** / **`is`/`is not`** / **`fn f(a, b = 3)`** / **`fn f(a, ...xs)`** / **`(a, b = 3) =>`** / **`(a, ...xs) =>`** / **`class` method `fn add(a, b = 3)`** / **`fn rest(a, ...xs)`** / **`{ add(a, b = 3) {} }`** / **`{ rest(a, ...xs) {} }`** / **`trait` default `fn add(a, b = 3)`** / **`fn rest(a, ...xs)`** / **`o?.x`** / **`xs?.[0]`** / **`f?.()`** / **`delete o.z`** / **`delete o.a.b`** / **`delete xs[0].x`** / **`delete o[k]`** / **`delete o.items[0].x`** / **`delete xs[0][0].x`** / **`delete this.z`** / **`delete o.items[0][0].x`** / **`delete o.a.b.c`** / **`delete this.a.b`** / **`delete o[k].x`** / **`delete super.z`** / **`delete this[k]`** / **`delete super[k]`** / **`delete super.a.b`** / **`delete o[k][j]`** / **`delete super.a[k]`** / **`delete this[k].x`** / **`delete super[k].x`** / **`delete this.a[k]`** / **`delete o.a[k]`** / **`delete this[k][j]`** / **`delete super[k][j]`** / **`delete o.items[0][k]`** / **`delete this.a.b[k]`** / **`delete o.a.b[k]`** / **`delete xs[0][0][k]`** / **`delete super.a.b[k]`** / **`delete this.items[0][k]`** / **`delete super.items[0][k]`** / **`delete this.items[0][0][k]`** / **`delete o.items[0][0][k]`** / **`delete super.items[0][0][k]`** / **`n &= 3`** / **`n |= 2`** / **`n ^= 3`** / **`o.x &= 3`** / **`xs[0] |= 2`** / **`o.x ^= 3`** / **`this.n &= 3`** / **`xs[0] ^= 3`** / **`super.n |= 2`** / **`o.x |= 2`** / **`xs[0] &= 3`** / **`this.n |= 2`** / **`this.n ^= 3`** / **`super.n &= 3`** / **`super.n ^= 3`** / **`o.a.b &= 3`** / **`o.a.b |= 2`** / **`o.a.b ^= 3`** / **`xs[0].x &= 3`** / **`xs[0].x |= 2`** / **`xs[0].x ^= 3`** / **`o.items[0] &= 3`** / **`o.items[0] |= 2`** / **`o.items[0] ^= 3`** / **`o.items[0][0] &= 3`** / **`o.items[0][0] |= 2`** / **`o.items[0][0] ^= 3`** / **`xs[0][0].x &= 3`** / **`xs[0][0].x |= 2`** / **`xs[0][0].x ^= 3`** / **`xs[0][0] &= 3`** / **`xs[0][0] |= 2`** / **`xs[0][0] ^= 3`** / **`o.items[0][0].x &= 3`** / **`o.items[0][0].x |= 2`** / **`o.items[0][0].x ^= 3`** / **`xs[0][0][0] &= 3`** / **`xs[0][0][0] |= 2`** / **`xs[0][0][0] ^= 3`** / **`n <<= 1`** / **`n >>= 1`** / **`n >>>= 1`** / **`o.x <<= 1`** / **`o.x >>= 1`** / **`o.x >>>= 1`** / **`xs[0] <<= 1`** / **`xs[0] >>= 1`** / **`xs[0] >>>= 1`** / **`this.n <<= 1`** / **`this.n >>= 1`** / **`this.n >>>= 1`** / **`super.n <<= 1`** / **`super.n >>= 1`** / **`super.n >>>= 1`** / **`o.a.b <<= 1`** / **`o.a.b >>= 1`** / **`o.a.b >>>= 1`** / **`xs[0].x <<= 1`** / **`xs[0].x >>= 1`** / **`xs[0].x >>>= 1`** / **`o.items[0] <<= 1`** / **`o.items[0] >>= 1`** / **`o.items[0] >>>= 1`** / **`o.items[0][0] <<= 1`** / **`o.items[0][0] >>= 1`** / **`o.items[0][0] >>>= 1`** / **`xs[0][0].x <<= 1`** / **`xs[0][0].x >>= 1`** / **`xs[0][0].x >>>= 1`** / **`o.items[0][0].x <<= 1`** / **`o.items[0][0].x >>= 1`** / **`o.items[0][0].x >>>= 1`** / **`xs[0][0] <<= 1`** / **`xs[0][0] >>= 1`** / **`xs[0][0] >>>= 1`** / **`xs[0][0][0] <<= 1`** / **`xs[0][0][0] >>= 1`** / **`xs[0][0][0] >>>= 1`** / **`n **= 2`** / **`o.x **= 2`** / **`xs[0] **= 2`** / **`this.n **= 2`** / **`super.n **= 2`** / **`o.a.b **= 2`** / **`xs[0].x **= 2`** / **`o.items[0] **= 2`** / **`o.items[0][0] **= 2`** / **`xs[0][0].x **= 2`** / **`o.items[0][0].x **= 2`** / **`xs[0][0] **= 2`** / **`xs[0][0][0] **= 2`** / **`n %= 7`** / **`o.x %= 7`** / **`xs[0] %= 7`** / **`this.n %= 7`** / **`super.n %= 7`** / **`o.a.b %= 7`** / **`xs[0].x %= 7`** / **`o.items[0] %= 7`** / **`o.items[0][0] %= 7`** / **`xs[0][0].x %= 7`** / **`o.items[0][0].x %= 7`** / **`xs[0][0] %= 7`** / **`xs[0][0][0] %= 7`** / **`n -= 2`** / **`o.x -= 2`** / **`xs[0] -= 2`** / **`this.n -= 2`** / **`super.n -= 2`** / **`o.a.b -= 2`** / **`xs[0].x -= 2`** / **`o.items[0] -= 2`** / **`o.items[0][0] -= 2`** / **`xs[0][0].x -= 2`** / **`o.items[0][0].x -= 2`** / **`xs[0][0] -= 2`** / **`xs[0][0][0] -= 2`** / **`n *= 3`** / **`switch`** / **`fallthrough`** / **`do while`** / **`xs[0] +=`** / **`step()?`/`bad()?`** / **`? :`** / **`import.meta`** / **`` `n=${n}` ``** / **`||=` `&&=` `??=`** / **`??`** / **`for let i = 0`** / **`for x of`** / **`for k in`** / **`match 1 { 1 => 2, _ => 0 }`** / **`match [x, y]`** / **`match { p, q }`** / **`if let Some(x)`** / **`while let Ok(v)`** / **`match 1..=5`** / **`n @ 1..=5`** / **`1 | 2 | 3`** / **`..5`** / **`5..`** / **`[h, ...t]`** / **`{ k, ...s }`** / **`[h, ...mid, last]`** / **`n @ 1..=5 if n != 3`** / **`Color.Red`** / **`Msg.Move(p)`** / **`xs @ [p, q]`** / **`wrap @ { k, ...s }`** / **`{ k: n @ 1..=5 }`** / **`[n @ 1, ...r]`** / **`Ok(n @ 1..=5)`** / **`Some(n @ 1..=5)`** / **`if let 1 | 2`** / **`while let 1 | 2`** / **`(1 | 2)`** / **`Option.Some(n)`** / **`Option.Some("x")`** / **`Option<Number>.None`** / **`1.0..=2.0`** / **`Result.Ok(n)`** / **`Result<Number, String>.Err`** / **`if let n @ Some(x)`** / **`if let 1.. = x`** / **`while let 1.. = r`** / **`if let ..5 = x`** / **`while let ..5 = r`** / **`n @ 1 | 2`** / **`v @ Msg.Move(x)`** / **`struct Box<T>`** / **`Box$String`** / **`echo$Number`** / **`echo$String`** / **`Box<String>("hi")`** / **`Child$Number`** / **`id<Number>(42)`** / **`id$String`** / **`id(id(42))`** / **`pair$Number_String`** / **`id$Box`** / **`pair(x, s)`** / **`len(wrap(1))`** / **`super.init`** / **`super.count = 1`** / **`super.n += 2`** / **`let m = super.tag`** / **`this.run(super.f)`** / **`Show<Number>`** / **`type Item = Number`** / **`where T: Show`** / **`show_it<Shown>`** / **`Box().show_it<Shown>`** / **`show_it<Nope>`** / **`Box().show_it<Nope>`** / **`Box<Nope>`** / **`where T: Show, T: Named`** / **`both_it<OnlyShow>`** / **`where A: Show, B: Named`** / **`pair_it<Shown, Nope>`** / **`PairBox<Shown, Labeled>`** / **`PairBox<Shown, Nope>`** / **`Box().join_ab<Shown, Labeled>`** / **`Box().join_ab<Shown, Nope>`** / **`Box().both_it<Shown>`** / **`Box().both_it<OnlyShow>`** / **`BothBox<Shown>`** / **`BothBox<OnlyShow>`** / **`WBox<Shown>`** / **`WBox<Nope>`** / **`Thing().id()`** / **`id() { return 42 }`** / **`Show<T> default`** / **`Show<T> default override`** / **`is(obj, "Class")`** / **`pass`/`assert`/`not`** / **`raise`** / **`o.x +=`** / **`o.a.b +=`** / **`o.items[0] +=`** / **`o.items[0][0] +=`** / **`xs[0].x +=`** / **`xs[0][0].x +=`** / **`o.items[0][0].x +=`** / **`xs[0][0] +=`** / **`xs[0][0][0] +=`** / **`o.x ||= `** / **`o.x &&=`** / **`o.x ??=`** / **`xs[0] ||= `** / **`xs[0] ??=`** / **`o.a.b ??=`** / **`o.items[0] ||= `** / **`xs[0].x ??=`** / **`xs[0][0] ||= `** / **`this.n ||= `** / **`o.items[0][0] ||= `** / **`xs[0][0].x ??=`** / **`super.n ||= `** / **`o.items[0][0].x ??=`** / **`xs[0][0][0] ||= `** / **`Child<T> super.n ||= `**; **kbcb v2** + **`op.tag`→leaf** + **Uint8Array/SAB** + **file-mmap**; **100** loop / **200** unrolled; `object_array_push`; 1k+ nested-interpreter-bundet; text-`maxKbc` 256KiB; **`maxKbcb` 8MiB**; **`get_member`+`call`**; **`get_super_method`** |
...
| **SH12** | **Låg-allok compile-hotpath** — återanvänd `sess`-buffertar (tokens/ops/out); färre temporära strängar/arrayer/objekt per token. **Inte** “zero-alloc” i default-Kab. Efter SH2 + P10a-profil | `allocs` / fas-ms ner på medium-fil vs samma baseline; ingen ny shard | ✅ subset: reuse `gSess`/`gE`/`gLxSess`; lexer `src` locals; in-place tokens; `bump` caches `pToksLen` |
...
| **SH12** | **Låg-allok compile-hotpath** — återanvänd `sess`-buffertar (tokens/ops/out); färre temporära strängar/arrayer/objekt per token. **Inte** “zero-alloc” i default-Kab. Efter SH2 + P10a-profil | `allocs` / fas-ms ner på medium-fil vs samma baseline; ingen ny shard | ✅ subset (`object_array_*`; `gLxSess` tokens; lexer `src` locals) |
...
| **SH16** | **Stäng Rust-compile för appar** | CI `KABOOTAR_COMPILE=self-host`; rust-fallback **failar** för app-`.kab` | ✅ `run`/`compile --rust` / rust-env vägrar appar; toolchain `self_host/` får rust; `sh16_app_run_refuses_rust_env` |
| **SH17** | **JIT i Kabootar** — register-alloc + native-emit som `.kab` (ersätt Cranelift i `src/`). Får börja som subset (i64-loopar). Bred teknik: [Våg FT](#våg-ft--fart-alla-tekniker-i-kab) F7–F10 | Hot loop kompilerad av Kab-JIT; `src/` JIT-moduler raderas när smoke är grön | ✅ subset smoke-complete: templates + map/page + `jitMmapOk` + `os_mm_store`/`os_mm_call` loop8/loopN/arith-imm/bit-ops/shifts/unary/eq/ne/lt/gt/le/ge/test/je/jne/jmp/jl/jle/jg/jge/nop dual-bind; exec deepen utan ny Cranelift/RWX i Rust |
| **SH18** | **GC i Kabootar** — nursery/sweep som `.kab` (ersätt Rust-GC) | Frame-budget smoke utan `src/` GC; sedan radera | ✅ subset smoke-complete: bump + promote + sweep + barrier + `gcMarkStep` + `gc_load`→`gc_concurrent_stress`→`gc_chain`→`gc_capstone` + `gcHostDeleteOk` dual-bind + `gcTlabHostDropOk`; F12 `@manual` `escManualReleaseOk` |
| **SH19** | **Laddare i Kab** — ingen `main.rs` som produkt. `.kab` startar runtime | `kabootar`-binär = Kab-bootstrap eller Kab-AOT | ✅ subset smoke-complete: `loadIsKab`/`loadEntry`/`loadIsKbc` + `load_aot`→`load_aot_capstone` AOT bootstrap + `wave_capstone` exec-smokes; `loadMainDeleteOk` dual-bind; radera `main.rs` deepen |
| **SH20** | **Stdlib i Kab** — sträng, array, objekt, math, JSON, datum, regex, collections som `.kab` (ersätt `src/runtime/stdlib`) | Smoke utan Rust-natives för kärn-API; radera motsvarande `.rs` | ✅ subset: `stdAdd`/`stdLen`/`stdHas` + `stdJsonIsNull` + `stdDateEpochOk` + `stdReHit` + `stdMul` + `stdObjGet` + `stdColPair` + `stdColGet` + `stdColLen` + `stdColPush` + `stdColPop` + `stdColFirst` + `stdColLast` + `stdColRest` + `stdColEmpty` + `stdColConcat` + `stdColRev` + `stdColContains` + `stdColIndex` + `stdColCount` + `stdColTake` + `stdColDrop` + `stdColZip` + `stdColUnzip` + `stdColFlat` + `stdColUnique` + `stdColEq` + `stdColClone` + `stdColRepeat` + `stdColFill` + `stdColRange` + `stdColSum` + `stdColMax` + `stdColMin` + `stdColProduct` + `stdColAvg` + `stdColMedian` + `stdColMode` + `stdColSort` + `stdColSortDesc` + `stdColFind` + `stdColFindLast` + `stdColRIndex` + `stdColSlice` + `stdColWindow` + `stdColChunk` + `stdColRotate` + `stdColPad` + `stdColInterleave` + `stdColTranspose` + `stdColDiag` + `stdColIdent` + `stdColTrace` + `stdColRow` + `stdColCol` + `stdColShape` + `stdColReshape` + `stdColDot` + `stdColMatVec` + `stdColMatMul` + `stdColOuter` + `stdColCross` + `stdColDet` + `stdColNorm` + `stdColUnit` + `stdColProj` + `stdColRej` + `stdColDist` + `stdColLerp` + `stdColScale` + `stdColAdd` + `stdColSub` + `stdColMul` + `stdColDiv` + `stdColNeg` + `stdColAbs` + `stdColSign` + `stdColClamp` + `stdColMod` + `stdColPow` + `stdColSqrt` + `stdColSqr` + `stdColCub` + `stdColFloor` + `stdColCeil` + `stdColRound` + `stdColTrunc` + `stdColLog` + `stdColLog2` + `stdColLog10` + `stdColExp` + `stdColSin` + `stdColCos` + `stdColTan` + `stdColAsin` + `stdColAcos` + `stdColAtan` + `stdColAtan2` + `stdColHypot` + `stdColCbrt` + `stdColImul` + `stdColClz32` + `stdColFround` + `stdColF16round` + `stdColSumPrecise` + `stdColLog1p` + `stdColExpm1` + `stdColSinh` + `stdColCosh` + `stdColTanh` + `stdColAsinh` + `stdColAcosh` + `stdColAtanh` + `stdColFmod` + `stdColRandom` + `stdColPi` + `stdColE` + `stdColLn2` + `stdColLn10` + `stdColLog2e` + `stdColLog10e` + `stdColSqrt2` + `stdColSqrt12`; radera natives deepen |
| **SH21** | **OS/FS/process i Kab** — `import "os"` policy + I/O i `.kab` (kOS). Rust `runtime/os` skuld | App läser/skriver filer via Kab-OS; host-FS bara kapabilitet tills drivrutin är Kab | ✅ subset: `kabOsIsVfs` / caps + `kabOsIsFile` + `kabOsArgvOk` + `kabOsEnvOk` + `kabOsCwdOk` + `kabOsIsDir` + `kabOsJoin` + `kabOsBase` + `kabOsExt` + `kabOsDirname` + `kabOsNorm` + `kabOsAbs` + `kabOsRel`; `kos/vfs` + radera `runtime/os` deepen |
| **SH22** | **SQL i Kab** — query/storage i `.kab` (ersätt `src/sql`) | `sql()` smoke utan Rust-motor | ✅ subset: `sqlIsSelect` / `sqlScalarOne` + `sqlIsWhere` + `sqlStoreOk` + `sqlIsLimit` + `sqlIsOrder` + `sqlIsInsert` + `sqlIsUpdate` + `sqlIsDelete` + `sqlIsCreate` + `sqlIsJoin` + `sqlIsGroup` + `sqlIsHaving` + `sqlIsDistinct` + `sqlIsUnion`; radera `src/sql` deepen |
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | ✅ subset: `cryptoIsHttps` / pin/trust + `cryptoTls12Ok` + `cryptoRootPem` + `cryptoTls13Ok` + `cryptoSha256Ok` + `cryptoHmacOk` + `cryptoAes256Ok` + `cryptoChaChaOk` + `cryptoGcmOk` + `cryptoEd25519Ok` + `cryptoX25519Ok`; rustls-delete deepen |
| **SH24** | **HTTP i Kab** — server/fetch ovanpå SH21/SH23 | `http_fetch_async` / `http_serve` utan `src/runtime/http.rs` | ✅ subset: host wraps + `httpIsPost` + `httpIsJson` + `httpIsPut` + `httpIsPatch` + `httpIsHead` + `httpIsDelete` + `httpIsOptions` + `httpIsTrace` + `httpIsConnect` + `httpWorkDropOk`; F15 same-room deepen |
...
- **Cacha `src`/`srcLen` som lokaler i skip/ident/number** — ✅ SH12 i `lexer_scan` (lexern är en fil).
...
**Nästa steg (bara `.kab`):** F10 frame-local lowering och x64/arm64 bytecode-roundtrip ✅; relocation/symbol-adresser, sektionsbindning, bytecode-bindning, serialiserade poster, tabellimage-varianter, table-apply till patchad adress, `relocword`-suffix, table-apply in i text-sektionens maskinkodshex, load/persist/verify/ship, ship-/verify-reject, relocation-ord på text-offset 32 med nollutfyllnad, load/persist/verify/ship av offset-32-text-imagen, ship-/verify-reject, båda relocation-orden (offset 32 och 40), load/persist/verify/ship av offset-40-text-imagen, ship-/verify-reject, bind av de två relocation-orden mot 4128 och 4136, dual-bind-reject för negativ base, dual-bind-reject för fel target på offset 40, dual-bind via `aotRelocSymbolBindOk` mot 4128 och 4136, dual-bind av text-hex mot de samma adresserna, dual-bind-hex-reject för fel base, dual-bind-hex-reject för fel tabell, dual-bind av `ToTextAt40` mot `WordHexAt32`/`WordHexAt40`, dual-bind av `ToImageTextAt40` mot samma word-hex, dual-bind-load av offset-40-imagen mot word-hex, dual-bind-verify av offset-40-imagen mot word-hex, dual-bind-ship av offset-40-imagen mot word-hex, dual-bind via `aotLoadedImageOk` av offset-40-imagen mot word-hex, dual-bind av loaded offset-40-image mot `aotRelocSymbolBindOk` (4128/4136), dual-bind av loaded offset-40-image mot `aotShipOk` och `aotRelocSymbolBindOk` tillsammans, dual-bind av `aotVerifyFullOk` tillsammans med loaded+ship+bind, dual-bind av `aotLoadImageWithRelocTextAt40Ok` tillsammans med verify+ship+bind, samt serialiserad reloc-tabell mot **rodata** ✅; table-apply-adress för rodata-reloc (base + 0) ✅; table-apply av rodata-ordet in i imagen (`relocword:0010000000000000`) ✅; load/persist/verify/ship av rodata-patched imagen ✅; ship-/verify-reject för rodata-patched imagen ✅; table-apply av rodata-ordet in i rodata-sektionens hex ✅; table-apply av rodata-hex in i native-imagen ✅; load/persist/verify/ship av rodata-hex-imagen ✅; ship-/verify-reject för rodata-hex-imagen ✅; dual-bind av rodata-hex-imagen mot patched adress 4096 ✅; dual-bind via `aotRelocSymbolBindOk` mot 4096 ✅; dual-bind-reject för negativ base på rodata-adressen ✅; dual-bind-reject för fel base (≠ 4096) på rodata-hex ✅; dual-bind-reject för fel tabell på rodata-hex ✅; dual-bind-load av rodata-hex-imagen mot WordHexRodata och AddressRodata ✅; dual-bind-verify av rodata-hex-imagen mot samma word-hex ✅; dual-bind-ship av rodata-hex-imagen mot samma word-hex ✅; dual-bind via `aotLoadedImageOk` av rodata-hex-imagen mot word-hex ✅; dual-bind av loaded rodata-hex-image mot `aotRelocSymbolBindOk` (4096) ✅; dual-bind av loaded rodata-hex-image mot `aotShipOk` och `aotRelocSymbolBindOk` tillsammans ✅; dual-bind av `aotVerifyFullOk` tillsammans med loaded+ship+bind för rodata-hex ✅; dual-bind av `aotLoadImageWithRelocRodataHexOk` tillsammans med verify+ship+bind ✅; serialiserad reloc-tabell mot **data** ✅; table-apply-adress för data-reloc (base + 0) ✅; table-apply av data-ordet in i imagen (`relocword:0010000000000000`) ✅; load/persist/verify/ship av data-patched imagen ✅; ship-/verify-reject för data-patched imagen ✅; table-apply av data-ordet in i data-sektionens hex ✅; table-apply av data-hex in i native-imagen ✅; load/persist/verify/ship av data-hex-imagen ✅; ship-/verify-reject för data-hex-imagen ✅; dual-bind av data-hex-imagen mot patched adress 4096 ✅; dual-bind via `aotRelocSymbolBindOk` mot 4096 ✅; dual-bind-reject för negativ base på data-adressen ✅; dual-bind-reject för fel base (≠ 4096) på data-hex ✅; dual-bind-reject för fel tabell på data-hex ✅; dual-bind-load av data-hex-imagen mot WordHexData och AddressData ✅; dual-bind-verify av data-hex-imagen mot samma word-hex ✅; dual-bind-ship av data-hex-imagen mot samma word-hex ✅; dual-bind via `aotLoadedImageOk` av data-hex-imagen mot word-hex ✅; dual-bind av loaded data-hex-image mot `aotRelocSymbolBindOk` (4096) ✅; dual-bind av loaded data-hex-image mot `aotShipOk` och `aotRelocSymbolBindOk` tillsammans ✅; dual-bind av `aotVerifyFullOk` tillsammans med loaded+ship+bind för data-hex ✅; dual-bind av `aotLoadImageWithRelocDataHexOk` tillsammans med verify+ship+bind ✅; table-apply av både rodata-hex och data-hex i samma native-image ✅; load/persist/verify/ship av den kombinerade rodata+data-hex-imagen ✅; ship-/verify-reject för den kombinerade rodata+data-hex-imagen ✅; dual-bind av den kombinerade rodata+data-hex-imagen mot patched adress 4096 ✅; dual-bind via `aotRelocSymbolBindOk` mot 4096 för den kombinerade imagen ✅; dual-bind-reject för negativ base på den kombinerade adressen ✅; dual-bind-reject för fel base (≠ 4096) på den kombinerade hex-imagen ✅; dual-bind-reject för fel tabell på den kombinerade hex-imagen ✅; dual-bind-load av den kombinerade hex-imagen mot WordHexRodataData och AddressRodataData ✅; dual-bind-verify av den kombinerade hex-imagen mot samma word-hex ✅; dual-bind-ship av den kombinerade hex-imagen mot samma word-hex ✅; dual-bind via `aotLoadedImageOk` av den kombinerade hex-imagen mot word-hex ✅; dual-bind av loaded kombinerad hex-image mot `aotRelocSymbolBindOk` (4096) ✅; dual-bind av loaded kombinerad hex-image mot `aotShipOk` och `aotRelocSymbolBindOk` tillsammans ✅; dual-bind av `aotVerifyFullOk` tillsammans med loaded+ship+bind för den kombinerade hex-imagen ✅; dual-bind av `aotLoadImageWithRelocRodataDataHexOk` tillsammans med verify+ship+bind ✅; table-apply av text-hex tillsammans med rodata+data-hex i samma native-image ✅; load/persist/verify/ship av den kombinerade text+rodata+data-hex-imagen ✅; ship-/verify-reject för den kombinerade text+rodata+data-hex-imagen ✅; dual-bind av den kombinerade text+rodata+data-hex-imagen mot patched adress 4096 ✅; dual-bind via `aotRelocSymbolBindOk` mot 4096 för den kombinerade text+rodata+data-imagen ✅; dual-bind-reject för negativ base på den kombinerade text+rodata+data-adressen ✅; dual-bind-reject för fel base (≠ 4096) på den kombinerade text+rodata+data-hex-imagen ✅; dual-bind-reject för fel tabell på den kombinerade text+rodata+data-hex-imagen ✅; dual-bind-load av den kombinerade text+rodata+data-hex-imagen mot WordHexTextRodataData och AddressTextRodataData ✅; dual-bind-verify av den kombinerade text+rodata+data-hex-imagen mot samma word-hex ✅; dual-bind-ship av den kombinerade text+rodata+data-hex-imagen mot samma word-hex ✅; dual-bind via `aotLoadedImageOk` av den kombinerade text+rodata+data-hex-imagen mot word-hex ✅; dual-bind av loaded text+rodata+data-hex-image mot `aotRelocSymbolBindOk` (4096) ✅; dual-bind av loaded text+rodata+data-hex-image mot `aotShipOk` och `aotRelocSymbolBindOk` tillsammans ✅; dual-bind av `aotVerifyFullOk` tillsammans med loaded+ship+bind för den kombinerade text+rodata+data-hex-imagen ✅; dual-bind av `aotLoadImageWithRelocTextRodataDataHexOk` tillsammans med verify+ship+bind ✅; PGO warm-image (`aotPgoOk` → `aotPgoWarmImage`) ✅; load/persist/verify/ship av PGO-warm-imagen ✅; dual-bind av `aotPgoOk` tillsammans med load+verify+ship av PGO-warm-imagen ✅; dual-bind av `aotPgoOk` och `aotFpOk` till PGO-warm-imagen ✅; load/persist/verify/ship av PGO-fp-imagen ✅; dual-bind av `aotPgoOk` och `aotFpOk` tillsammans med load+verify+ship av PGO-fp-imagen ✅; dual-bind av PGO-fp-imagen mot kallstart-budget (`aotColdOk`) ✅; load/persist/verify/ship av PGO-fp-cold-imagen ✅; dual-bind av `aotPgoOk`, `aotFpOk` och `aotColdOk` tillsammans med load+verify+ship av PGO-fp-cold-imagen ✅; dual-bind av PGO-fp-cold-imagen mot steady-state-budget (`aotSteadyOk`) ✅; load/persist/verify/ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotPgoOk`, `aotFpOk`, `aotColdOk` och `aotSteadyOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotImageOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotImageOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotSealOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotSealOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotEntryOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotEntryOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotPageOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotPageOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotAlignOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotAlignOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotSectionOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotSectionOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotLoadManifestOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotVerifyImageOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotVerifyImageOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotVerifyFullOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotVerifyFullOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotShipOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotShipOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotLoadedImageOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotLoadedImageOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotLoadImageWithPgoWarmFpColdSsOk` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotLoadImageWithPgoWarmFpColdSsOk` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; capstone dual-bind av full native-image policy-kedjan tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅; dual-bind av `aotImageName` till PGO-fp-cold-ss-imagen ✅; dual-bind av `aotImageName` tillsammans med load+verify+ship av PGO-fp-cold-ss-imagen ✅ (ret-stub policy-kedjan komplett); dual-bind av `aotSymbolRecordOk`/`aotRelocRecordOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; load/persist/verify/ship av PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotImageOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotImageOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotSealOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotSealOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotEntryOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotEntryOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotPageOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotPageOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotAlignOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotAlignOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotSectionOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotSectionOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotTextProtectOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotTextProtectOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotRodataProtectOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotRodataProtectOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotDataProtectOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotDataProtectOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotTargetOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotTargetOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotAbiOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotAbiOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotLoadCodeOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotLoadCodeOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotVerifyCodeOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotVerifyCodeOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotLoadManifestOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotLoadManifestOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotVerifyImageOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotVerifyImageOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotVerifyFullOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotVerifyFullOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotShipOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotShipOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotLoadedImageOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotLoadedImageOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotLoadImageWithPgoWarmFpColdSsSymRelocOk` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotLoadImageWithPgoWarmFpColdSsSymRelocOk` tillsammans med load+verify+ship av sym/reloc-varianten ✅; capstone dual-bind av full native-image policy-kedjan tillsammans med load+verify+ship av sym/reloc-varianten ✅; dual-bind av `aotImageName` till PGO-fp-cold-ss sym/reloc-imagen ✅; dual-bind av `aotImageName` tillsammans med load+verify+ship av sym/reloc-varianten ✅ (sym/reloc policy-kedjan komplett); dual-bind av `nollAotReady` till ret-stub och sym/reloc capstones ✅ (F10 AOT native-image policy komplett). **SH17–SH19 smoke-complete:** JIT `os_mm_store`/`os_mm_call` loop8/loopN/arith-imm/bit-ops/shifts/unary/eq/ne/lt/gt/le/ge; GC `gc_load`→`gc_chain`→`gc_capstone` + concurrent mark/stress; AOT bootstrap `load_aot`→`load_aot_capstone` + `wave_capstone`; capstone exec-smokes (gate rollup compile+eval i `tests/sh_wave.rs`). `nollAotReady` förblir false. **Radera inte `src/`.** Inga nya `src/*.rs`.
...
**Nästa:** Compound assign är **en** emit-väg. Behåll eval-once. **Vägg 1–5** + iterator-protokoll + `g.throw` `catch`/`finally`/`yield`/`yield*`/`send` klara. **`return` i `finally` ersätter uppskjutna `return` från `catch`** (`…finally_override_ok` → **894**). **`throw` i `finally` ersätter den uppskjutna `return` från `catch`** (`…finally_throw_override_ok` → **894**). **`g.throw` in i `finally`-`yield`** (`…throw_into_yield_ok` → **894**). **Nästlad `try` + `g.throw` in i inner `finally`-`yield`** (`sh6_self_host_generator_nested_try_throw_into_finally_yield_ok` → **194**). **`g.throw` in i inner `try`** kör inner `finally`-`yield` sedan yttre `catch` (`sh6_self_host_generator_nested_try_throw_into_try_ok` → **194**). **`g.return` in i inner `try`** kör inner `finally`-`yield` sedan completion (`sh6_self_host_generator_nested_try_return_into_try_ok` → **194**). **`g.return` in i inner `finally`-`yield`** (`sh6_self_host_generator_nested_try_return_into_finally_yield_ok` → **194**). **Yttre `finally` + `g.throw`** (`sh6_self_host_generator_nested_try_outer_finally_throw_ok` → **1984**). **Yttre `finally` + `g.return`** (`sh6_self_host_generator_nested_try_outer_finally_return_ok` → **1984**). **`g.return` vid yttre `finally`-`yield`** (`sh6_self_host_generator_nested_try_return_into_outer_finally_yield_ok` → **1984**). **`g.throw` vid yttre `finally`-`yield`** (`sh6_self_host_generator_nested_try_throw_into_outer_finally_yield_ok` → **1984**). **`g.throw` vid inner `finally`-`yield` + yttre `finally`** (`sh6_self_host_generator_nested_try_throw_inner_finally_outer_finally_ok` → **1984**). **`g.return` vid inner `finally`-`yield` + yttre `finally`** (`sh6_self_host_generator_nested_try_return_inner_finally_outer_finally_ok` → **1984**). **`g.next(v)` send in i yttre `finally`-`yield`** (`sh6_self_host_generator_nested_try_outer_finally_send_ok` → **1984**). **`g.next(v)` send in i inner `finally`-`yield`** (`sh6_self_host_generator_nested_try_inner_finally_send_ok` → **1984**). **`g.next(v)` send in i inner `try`-`yield`** (`sh6_self_host_generator_nested_try_inner_try_send_ok` → **1984**). **`yield*` i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_outer_finally_ok` → **1984**). **`g.throw` genom `yield*` i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_throw_outer_finally_ok` → **1984**). **`g.return` genom `yield*` i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_return_outer_finally_ok` → **1984**). **`g.next(v)` send genom `yield*` i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_send_outer_finally_ok` → **1984**). **Send genom `yield*` när inner-`fn*` har `try/finally`** (`sh6_self_host_generator_nested_try_yield_star_inner_try_send_outer_finally_ok` → **17984**). **`g.throw` genom `yield*` när inner-`fn*` har `try/finally`** (`sh6_self_host_generator_nested_try_yield_star_inner_try_throw_outer_finally_ok` → **17984**). **`g.return` genom `yield*` när inner-`fn*` har `try/finally`** (`sh6_self_host_generator_nested_try_yield_star_inner_try_return_outer_finally_ok` → **17984**). **Send in i inner-`fn*`:s `finally`-`yield` via `yield*`** (`sh6_self_host_generator_nested_try_yield_star_inner_finally_send_outer_finally_ok` → **17984**). **`g.throw` in i inner-`fn*`:s `finally`-`yield` via `yield*`** (`sh6_self_host_generator_nested_try_yield_star_inner_finally_throw_outer_finally_ok` → **17984**). **`g.return` in i inner-`fn*`:s `finally`-`yield` via `yield*`** (`sh6_self_host_generator_nested_try_yield_star_inner_finally_return_outer_finally_ok` → **17984**). **`g.next(v)` send in i yttre `finally`-`yield` efter `yield*`** (`sh6_self_host_generator_nested_try_yield_star_outer_finally_send_ok` → **1984**). **`g.throw` vid yttre `finally`-`yield` efter `yield*`** (`sh6_self_host_generator_nested_try_yield_star_throw_into_outer_finally_yield_ok` → **1984**). **`g.return` vid yttre `finally`-`yield` efter `yield*`** (`sh6_self_host_generator_nested_try_yield_star_return_into_outer_finally_yield_ok` → **1984**). **Nästlad `yield* mid()`/`inner()` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_nested_outer_finally_ok` → **1984**). **`g.throw` genom nästlad `yield* mid()`/`inner()` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_nested_throw_outer_finally_ok` → **1984**). **`g.return` genom nästlad `yield* mid()`/`inner()` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_nested_return_outer_finally_ok` → **1984**). **`g.next(v)` send genom nästlad `yield* mid()`/`inner()` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_nested_send_outer_finally_ok` → **1984**). **`yield*` av array i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_array_outer_finally_ok` → **1984**). **`g.throw` genom `yield*` av array i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_array_throw_outer_finally_ok` → **1984**). **`g.return` genom `yield*` av array i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_array_return_outer_finally_ok` → **1984**). **`g.next(v)` send in i yttre `finally`-`yield` efter `yield*` av array** (`sh6_self_host_generator_nested_try_yield_star_array_outer_finally_send_ok` → **1984**). **`g.throw` vid yttre `finally`-`yield` efter `yield*` av array** (`sh6_self_host_generator_nested_try_yield_star_array_throw_into_outer_finally_yield_ok` → **1984**). **`g.return` vid yttre `finally`-`yield` efter `yield*` av array** (`sh6_self_host_generator_nested_try_yield_star_array_return_into_outer_finally_yield_ok` → **1984**). **`yield*` av `{ next() }` i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_custom_outer_finally_ok` → **1984**). **`g.throw` genom `yield*` av `{ next() }` (utan `throw`) + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_outer_finally_ok` → **1984**). **`g.return` genom `yield*` av `{ next() }` (utan `return`) + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_outer_finally_ok` → **1984**). **`g.next(v)` send in i yttre `finally`-`yield` efter `yield*` av `{ next() }`** (`sh6_self_host_generator_nested_try_yield_star_custom_outer_finally_send_ok` → **1984**). **`g.throw` vid yttre `finally`-`yield` efter `yield*` av `{ next() }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_into_outer_finally_yield_ok` → **1984**). **`g.return` vid yttre `finally`-`yield` efter `yield*` av `{ next() }`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_into_outer_finally_yield_ok` → **1984**). **`g.throw` genom `yield*` av `{ next(), throw() }` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_method_outer_finally_ok` → **1498**). **`g.return` genom `yield*` av `{ next(), return() }` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_method_outer_finally_ok` → **1498**). **`g.next(v)` send genom `yield*` av `{ next() }` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_custom_send_outer_finally_ok` → **1984**). **`yield*` av `Symbol.iterator` i inner `try` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_outer_finally_ok` → **1984**). **`g.throw` genom `yield*` av `Symbol.iterator` (utan `throw`) + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_outer_finally_ok` → **1984**). **`g.return` genom `yield*` av `Symbol.iterator` (utan `return`) + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_outer_finally_ok` → **1984**). **`g.next(v)` send genom `yield*` av `Symbol.iterator` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_send_outer_finally_ok` → **1984**). **`g.next(v)` send in i yttre `finally`-`yield` efter `yield*` av `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_outer_finally_send_ok` → **1984**). **`g.throw` vid yttre `finally`-`yield` efter `yield*` av `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_into_outer_finally_yield_ok` → **1984**). **`g.return` vid yttre `finally`-`yield` efter `yield*` av `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_into_outer_finally_yield_ok` → **1984**). **`g.throw` genom `yield*` av `Symbol.iterator` med `throw()` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_method_outer_finally_ok` → **1498**). **`g.return` genom `yield*` av `Symbol.iterator` med `return()` + yttre `finally`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_method_outer_finally_ok` → **1498**). **`g.throw` genom `yield*` av `{ next(), throw() }` där `throw()` kastar vidare** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_rethrows_outer_finally_ok` → **1984**). **`g.throw` genom `yield*` av `Symbol.iterator` vars `throw()` kastar vidare** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_rethrows_outer_finally_ok` → **1984**). **`g.return` genom `yield*` av `{ next(), return() }` där `return()` kastar** (`sh6_self_host_generator_nested_try_yield_star_custom_return_throws_outer_finally_ok` → **1984**). **`g.return` genom `yield*` av `Symbol.iterator` vars `return()` kastar** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_throws_outer_finally_ok` → **1984**). **`g.throw` genom `yield*` av `{ next(), throw() }` där `throw()` är `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_outer_finally_ok` → **1998**). **Femte `.next()` efter `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_outer_finally_return_ok` → **19984**). **`g.throw` genom `yield*` av `Symbol.iterator` vars `throw()` är `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_outer_finally_ok` → **1998**). **Femte `.next()` efter `Symbol.iterator` `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_outer_finally_return_ok` → **19984**). **`g.return` genom `yield*` av `{ next(), return() }` där `return()` är `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_outer_finally_ok` → **1998**). **Femte `.next()` efter `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_outer_finally_return_ok` → **19984**). **`g.return` genom `yield*` av `Symbol.iterator` vars `return()` är `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_outer_finally_ok` → **1998**). **Femte `.next()` efter `Symbol.iterator` `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_outer_finally_return_ok` → **19984**). **`g.next(v)` send in i inner `finally`-`yield` efter `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_inner_finally_send_ok` → **19981**). **`g.next(v)` send in i inner `finally`-`yield` efter `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_inner_finally_send_ok` → **19981**). **`g.next(v)` send in i inner `finally`-`yield` efter `Symbol.iterator` `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_inner_finally_send_ok` → **19981**). **`g.next(v)` send in i inner `finally`-`yield` efter `Symbol.iterator` `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_inner_finally_send_ok` → **19981**). **`g.throw` in i inner `finally`-`yield` efter `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_inner_finally_throw_ok` → **1999**). **Femte `.next()` efter `g.throw` in i inner `finally`-`yield`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_inner_finally_throw_next_ok` → **19998**). **`g.throw` in i inner `finally`-`yield` efter `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_inner_finally_throw_ok` → **1998**). **Femte `.next()` efter `g.throw` efter `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_inner_finally_throw_next_ok` → **19981**). **`g.throw` in i inner `finally`-`yield` efter `Symbol.iterator` `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_inner_finally_throw_ok` → **1999**). **Femte `.next()` efter `Symbol.iterator` `throw()` `done: true` + inner `g.throw`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_inner_finally_throw_next_ok` → **19998**). **`g.throw` in i inner `finally`-`yield` efter `Symbol.iterator` `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_inner_finally_throw_ok` → **1998**). **Femte `.next()` efter `Symbol.iterator` `return()` `done: true` + inner `g.throw`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_inner_finally_throw_next_ok` → **19981**). **`g.return` in i inner `finally`-`yield` efter `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_inner_finally_return_ok` → **1998**). **Femte `.next()` efter `g.return` efter `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_inner_finally_return_next_ok` → **19981**). **`g.return` in i inner `finally`-`yield` efter `Symbol.iterator` `throw()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_inner_finally_return_ok` → **1998**). **Femte `.next()` efter `Symbol.iterator` `throw()` `done: true` + inner `g.return`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_inner_finally_return_next_ok` → **19981**). **`g.return` in i inner `finally`-`yield` efter `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_inner_finally_return_ok` → **1931**). **Femte `.next()` efter `g.return` efter `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_inner_finally_return_next_ok` → **19311**). **`g.return` in i inner `finally`-`yield` efter `Symbol.iterator` `return()` `done: true`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_inner_finally_return_ok` → **1931**). **Femte `.next()` efter `Symbol.iterator` `return()` `done: true` + inner `g.return`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_inner_finally_return_next_ok` → **19311**). **`throw()` `{ done: false }` fortsätter `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_ok` → **1429**). **Femte `.next()` efter `throw()` `{ done: false }` fortsätter `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_next_ok` → **14298**). **`Symbol.iterator` `throw()` `{ done: false }` fortsätter `yield*`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_ok` → **1429**). **Femte `.next()` efter `Symbol.iterator` `throw()` `{ done: false }` fortsätter `yield*`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_next_ok` → **14298**). **`return()` `{ done: false }` avslutar `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_false_continue_ok` → **1498**). **Femte `.next()` efter `return()` `{ done: false }` avslutar `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_false_continue_next_ok` → **14981**). **`Symbol.iterator` `return()` `{ done: false }` avslutar `yield*`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_false_continue_ok` → **1498**). **Femte `.next()` efter `Symbol.iterator` `return()` `{ done: false }` avslutar `yield*`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_false_continue_next_ok` → **14981**). **`g.throw` in i fortsatt `yield*` efter `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_throw_ok` → **1423**). **Femte `.next()` efter andra `g.throw` in i fortsatt `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_throw_next_ok` → **14232**). **`g.throw` in i fortsatt `yield*` efter `Symbol.iterator` `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_throw_ok` → **1423**). **Femte `.next()` efter andra `g.throw` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_throw_next_ok` → **14232**). **`g.return` in i fortsatt `yield*` efter `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_return_ok` → **1429**). **Femte `.next()` efter `g.return` in i fortsatt `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_return_next_ok` → **14298**). **`g.return` in i fortsatt `yield*` efter `Symbol.iterator` `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_return_ok` → **1429**). **Femte `.next()` efter `g.return` via `Symbol.iterator` in i fortsatt `yield*`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_return_next_ok` → **14298**). **`g.return` in i fortsatt `yield*` med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_return_method_ok` → **1423**). **Femte `.next()` efter `g.return` med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_return_method_next_ok` → **14239**). **`g.return` via `Symbol.iterator` in i fortsatt `yield*` med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_return_method_ok` → **1423**). **Femte `.next()` efter `g.return` via `Symbol.iterator` med iterator-`return()`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_return_method_next_ok` → **14239**). **Sjätte `.next()` efter `g.return` med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_return_method_next6_ok` → **142398**). **Sjätte `.next()` efter `g.return` via `Symbol.iterator` med iterator-`return()`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_return_method_next6_ok` → **142398**). **`g.next(v)` send in i fortsatt `yield*` efter `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_ok` → **1479**). **`g.next(v)` send in i fortsatt `yield*` efter `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_ok` → **1479**). **Femte `.next()` efter send in i fortsatt `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_next_ok` → **14798**). **Femte `.next()` efter send in i fortsatt `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_next_ok` → **14798**). **`g.next(v)` send via `Symbol.iterator` in i fortsatt `yield*` efter `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_ok` → **1479**). **`g.next(v)` send via `Symbol.iterator` in i fortsatt `yield*` efter `throw()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_ok` → **1479**). **Femte `.next()` efter samma `Symbol.iterator`-send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_next_ok` → **14798**). **Femte `.next()` efter samma `Symbol.iterator`-send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_next_ok` → **14798**). **`g.next(v)` send efter `return()` `{ done: false }` som avslutar `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_false_continue_send_ok` → **1498**). **`g.next(v)` send efter `return()` `{ done: false }` som avslutar `yield*`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_false_continue_send_ok` → **1498**). **Femte `.next()` efter samma send som inte återupptar `it.next`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_false_continue_send_next_ok` → **14981**). **Femte `.next()` efter samma send som inte återupptar `it.next`** (`sh6_self_host_generator_nested_try_yield_star_custom_return_done_false_continue_send_next_ok` → **14981**). **Samma send via `Symbol.iterator` `return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_false_continue_send_ok` → **1498**). **Samma send via `Symbol.iterator` `return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_false_continue_send_ok` → **1498**). **Femte `.next()` efter samma `Symbol.iterator`-send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_false_continue_send_next_ok` → **14981**). **Femte `.next()` efter samma `Symbol.iterator`-send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_return_done_false_continue_send_next_ok` → **14981**). **Andra `g.throw` in i fortsatt `yield*` efter `g.next(v)` send** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_throw_ok` → **1473**). **Andra `g.throw` in i fortsatt `yield*` efter `g.next(v)` send** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_throw_ok` → **1473**). **Femte `.next()` efter andra `g.throw` efter send** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_throw_next_ok` → **14732**). **Femte `.next()` efter andra `g.throw` efter send** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_throw_next_ok` → **14732**). **Andra `g.throw` via `Symbol.iterator` efter `g.next(v)` send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_throw_ok` → **1473**). **Andra `g.throw` via `Symbol.iterator` efter `g.next(v)` send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_throw_ok` → **1473**). **Femte `.next(v)` efter andra `g.throw` via `Symbol.iterator` efter send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_throw_next_ok` → **14732**). **Femte `.next(v)` efter andra `g.throw` via `Symbol.iterator` efter send** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_throw_next_ok` → **14732**). **`g.return` in i fortsatt `yield*` efter `g.next(v)` send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_ok` → **1479**). **`g.return` in i fortsatt `yield*` efter `g.next(v)` send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_ok` → **1479**). **Femte `.next()` efter `g.return` efter send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_next_ok` → **14798**). **Femte `.next()` efter `g.return` efter send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_next_ok` → **14798**). **`g.return` via `Symbol.iterator` in i fortsatt `yield*` efter send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_ok` → **1479**). **`g.return` via `Symbol.iterator` in i fortsatt `yield*` efter send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_ok` → **1479**). **Femte `.next()` efter `g.return` via `Symbol.iterator` efter send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_next_ok` → **14798**). **Femte `.next()` efter `g.return` via `Symbol.iterator` efter send utan iterator-`return`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_next_ok` → **14798**). **`g.return` in i fortsatt `yield*` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_ok` → **1473**). **`g.return` in i fortsatt `yield*` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_ok` → **1473**). **Femte `.next()` efter `g.return` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_next_ok` → **14739**). **Femte `.next()` efter `g.return` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_next_ok` → **14739**). **`g.return` via `Symbol.iterator` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_ok` → **1473**). **`g.return` via `Symbol.iterator` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_ok` → **1473**). **Femte `.next()` efter `g.return` via `Symbol.iterator` efter send med iterator-`return()`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_next_ok` → **14739**). **Femte `.next()` efter `g.return` via `Symbol.iterator` efter send med iterator-`return()`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_next_ok` → **14739**). **Sjätte `.next()` efter `g.return` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_next6_ok` → **147398**). **Sjätte `.next()` efter `g.return` efter send med iterator-`return()` `{ done: false }`** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_next6_ok` → **147398**). **Sjätte `.next()` efter `g.return` via `Symbol.iterator` efter send med iterator-`return()`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_next6_ok` → **147398**). **Sjätte `.next()` efter `g.return` via `Symbol.iterator` efter send med iterator-`return()`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_next6_ok` → **147398**). **Sjunde `.next()` `done` efter samma custom send+`return()`-kedja** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_next7_ok` → **1473981**). **Sjunde `.next()` `done` efter samma custom send+`return()`-kedja** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_next7_ok` → **1473981**). **Sjunde `.next()` `done` via `Symbol.iterator` efter samma kedja** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_next7_ok` → **1473981**). **Sjunde `.next()` `done` via `Symbol.iterator` efter samma kedja** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_next7_ok` → **1473981**). **`g.throw` efter completion av samma custom-kedja** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_throw_done_ok` → **1473985**). **`g.throw` efter completion av samma custom-kedja** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_throw_done_ok` → **1473985**). **`g.throw` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_throw_done_ok` → **1473985**). **`g.throw` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_throw_done_ok` → **1473985**). **`g.return` efter completion av samma custom-kedja** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_ok` → **1473981**). **`g.return` efter completion av samma custom-kedja** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_ok` → **1473981**). **`g.return` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_ok` → **1473981**). **`g.return` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_ok` → **1473981**). **Extra `.next()` efter `g.throw` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_throw_done_next_ok` → **1473981**). **Extra `.next()` efter `g.throw` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_throw_done_next_ok` → **1473981**). **Extra `.next()` efter `g.throw` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_throw_done_next_ok` → **1473981**). **Extra `.next()` efter `g.throw` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_throw_done_next_ok` → **1473981**). **Extra `.next()` efter `g.return` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_next_ok` → **1473981**). **Extra `.next()` efter `g.return` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_next_ok` → **1473981**). **Extra `.next()` efter `g.return` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_next_ok` → **1473981**). **Extra `.next()` efter `g.return` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_next_ok` → **1473981**). **`g.throw` efter extra `.next()` efter `g.throw` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_throw_done_next_throw_ok` → **1473982**). **`g.throw` efter extra `.next()` efter `g.throw` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_throw_done_next_throw_ok` → **1473982**). **Samma `g.throw` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_throw_done_next_throw_ok` → **1473982**). **Samma `g.throw` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_throw_done_next_throw_ok` → **1473982**). **`g.return` efter extra `.next()` efter `g.return` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_next_return_ok` → **1473981**). **`g.return` efter extra `.next()` efter `g.return` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_next_return_ok` → **1473981**). **Samma `g.return` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_next_return_ok` → **1473981**). **Samma `g.return` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_next_return_ok` → **1473981**). **`g.throw` efter extra `.next()` efter `g.return` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_next_throw_ok` → **1473982**). **`g.throw` efter extra `.next()` efter `g.return` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_return_done_next_throw_ok` → **1473982**). **Samma `g.throw` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_next_throw_ok` → **1473982**). **Samma `g.throw` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_return_done_next_throw_ok` → **1473982**). **`g.return` efter extra `.next()` efter `g.throw` efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_throw_done_next_return_ok` → **1473981**). **`g.return` efter extra `.next()` efter `g.throw` efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_throw_done_next_return_ok` → **1473981**). **`g.next(v)` send efter completion av samma send+`return()`-kedja** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_send_done_ok` → **1473981**). **`g.next(v)` send efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_send_done_ok` → **1473981**). **Extra `.next()` efter `g.next(v)` send efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_send_done_next_ok` → **1473981**). **Extra `.next()` efter `g.next(v)` send efter completion via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_send_done_next_ok` → **1473981**). **`g.throw` efter extra `.next()` efter `g.next(v)` send efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_send_done_next_throw_ok` → **1473982**). **Samma `g.throw` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_send_done_next_throw_ok` → **1473982**). **`g.return` efter extra `.next()` efter `g.next(v)` send efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_send_done_next_return_ok` → **1473981**). **Samma `g.return` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_send_done_next_return_ok` → **1473981**). **`g.throw` direkt efter `g.next(v)` send efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_send_done_throw_ok` → **1473982**). **Samma `g.throw` via `Symbol.iterator`** (`sh6_self_host_generator_nested_try_yield_star_symbol_iterator_throw_done_false_continue_send_return_method_send_done_throw_ok` → **1473982**). **`g.return` direkt efter `g.next(v)` send efter completion (custom)** (`sh6_self_host_generator_nested_try_yield_star_custom_throw_done_false_continue_send_return_method_send_done_return_ok` → **1473981**). **Nästa:** samma `g.return` via `Symbol.iterator`. Inte `async fn*`. Inte SH17–SH28. Inte höja text-`maxKbc`. Inte `noll_*`. Inte rust-opcode för `is` eller rust-compile av default/rest-params.
