...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 policy-subset: `cryptoIsHttps` / pin/trust + TLS/alg-gates; testvägar verifierar rätt `lib/kab/crypto/`-blad. **Nästa:** riktig SHA-256-eval i Kab, sedan HMAC/TLS; rustls-delete först därefter |
...
**SH6 = KLAR** mot stoppkriteriet (appar + using). **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** `.kbc`/`.kbcb` disk-eval. **SH20** JSON parse/stringify. **SH21** join+norm+write+read. **SH22 deepen:** `sqlExecOk` INSERT+SELECT eq (`sh22_sql_exec_smoke`). **SH23 policy-gates verifierade mot rätt blad; self-host shift-parsern fångar nu operator/vänster/högerled (`sh23_self_host_shift_high_word_ok`) och emitterar `bit_or` för kombinerade skift (`sh23_self_host_shift_or_emits_bit_or`). Nästa: isolera körningens stackväg, sedan SHA-256-eval i Kab** (inte fler flagg-kloner). Inte async fn*. Inte fler sqlIs*-kloner.
