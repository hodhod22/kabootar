...
| **F11** | **Nursery GC i Kab** (SH18) — bump, promote, frame-budget; **TLAB** (F22) när workers (F13) är riktiga | 60 FPS utan GC-spike i idle; alloc utan globalt lock per objekt | ✅ subset: bump + promote + sweep + barrier + `gcMarkStep` + `gc_load`→`gc_chain`→`gc_capstone` + concurrent mark/stress + `gcHostDeleteOk` dual-bind + `gcTlabNurseryOk` + `gcTlabPauseOk` + `gcTlabHostDropOk` + deep-löv (tri/incr/rem/copy/sweep/manual, barnskuggad worklist) + promote-boundary fix + deep-6 (`gc_lazy`/`gc_idle`/`gc_minor` → `gcDeep6Ok`) + lazy sweep i Kab-VM (`vGcSweepSliceS`) + deep-7 (generationer: minor/major via `gcOldId`+rem-set, alloc-black, idle-drain `vGcIdleS`, charge på literal-ops) + deep-8 (survival-pacing `gcMajorDue`, barriär på merge/writeback + `curGen`/`genClose`-roots); F12 `@manual` `escManualReleaseOk` |
