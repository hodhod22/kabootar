...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 SHA-256 FIPS + HMAC-SHA256 RFC 4231 + TLS 1.2 P_SHA256 PRF/Finished eval (`sh23_crypto_sha_eval_smoke`, `sh23_crypto_hmac_eval_smoke`, `sh23_crypto_tls_prf_eval_smoke`); rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** handshake-records/cert-verify i Kab, därefter rustls-delete |
...
**SH6 = KLAR** mot stoppkriteriet (appar + using). **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** `.kbc`/`.kbcb` disk-eval. **SH20** JSON parse/stringify. **SH21** join+norm+write+read. **SH22 deepen:** `sqlExecOk` INSERT+SELECT eq (`sh22_sql_exec_smoke`). **SH23 SHA-256 FIPS + HMAC-SHA256 RFC 4231 + TLS 1.2 P_SHA256 PRF/Finished eval i Kab (`sh23_crypto_sha_eval_smoke`, `sh23_crypto_hmac_eval_smoke`, `sh23_crypto_tls_prf_eval_smoke`). rustls-host kvar. Nästa: handshake-records/cert-verify i Kab, därefter rustls-delete** (inte fler flagg-kloner). Inte async fn*. Inte fler sqlIs*-kloner.
