...
| **SH6** | **Kab-VM som produktväg** — inte evig bootstrap | `KABOOTAR_VM=kab-only` default-smoke; `vm_*` **< 40** | ✅ subset: **kab-only default**; **`.kbcb` run** via mmap; **`in`**; **`await`**; **`array_slice_from`**; **`new_instance_from_array`**; self-host **`let [a, ...rest]`** / **`let { a, ...rest }`** / **`let { x: [a, b] }`** / **`[1, ...xs]`** / **`{ ...obj }`** / **`{ a }`** / **`{ foo() {} }`** / **`{ [k]: v }`** / **`is`/`is not`** / **`fn f(a, b = 3)`** / **`fn f(a, ...xs)`** / **`(a, b = 3) =>`** / **`(a, ...xs) =>`** / **`class` method `fn add(a, b = 3)`** / **`fn rest(a, ...xs)`** / **`{ add(a, b = 3) {} }`** / **`{ rest(a, ...xs) {} }`** / **`trait` default `fn add(a, b = 3)`** / **`fn rest(a, ...xs)`** / **`o?.x`** / **`xs?.[0]`** / **`f?.()`** / **`delete o.z`** / **`switch`** / **`fallthrough`** / **`do while`** / **`xs[0] +=`** / **`step()?`/`bad()?`** / **`? :`** / **`import.meta`** / **`` `n=${n}` ``** / **`||=` `&&=` `??=`** / **`??`** / **`for let i = 0`** / **`is(obj, "Class")`** / **`pass`/`assert`/`not`** / **`raise`** / **`o.x +=`** / **`o.a.b +=`** / **`o.items[0] +=`** / **`xs[0].x +=`** / **`xs[0][0] +=`**; **kbcb v2** + **`op.tag`→leaf** + **Uint8Array/SAB** + **file-mmap**; **100** loop / **200** unrolled; `object_array_push`; 1k+ nested-interpreter-bundet; text-`maxKbc` 256KiB; **`maxKbcb` 8MiB**; **`get_member`+`call`**; **`get_super_method`** |
...
**Nästa:** SH6 self-host **`xs[0][0] +=`** är proven på Kab-VM (`1+8` → 9; IndexAssign med Index-objekt store-back via `index_set` + `iatmp`). **`xs[0][0][0] +=`** (djupare än Index-av-Index) poppar fortfarande utan ytter-store-back. `using x = r` är self-host parse+emit + rust-VM; **inte** Kab-VM — `store_global` från anropad fn syns inte i anroparen (trampolin-COW; ny native/trampolin är ny arkitektur). `with` samma vägg. `import("math")` / `dynamic_import` är self-host+rust-VM; guest-import under `KAB_VM_EXEC_ACTIVE` går via host `eval_program` (inte Kab-VM-proven). `async fn` är ny arkitektur (Kab-VM slänger `fn_async`; rust `schedule_bytecode_async`). `for await` behöver en native som returnerar uppdaterad iterator (inte ny native nu). `yield` är generator-arkitektur. 1k+ loop är SH17/JIT. Inte höja text-`maxKbc`. Inte `noll_*`. Inte SH17–SH28. Inte rust-opcode för `is` eller rust-compile av default/rest-params.
