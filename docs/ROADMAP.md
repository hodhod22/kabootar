...
| **Nästa kod** | rustls-host är kvar (`cryptoHostDeleteOk` false). Kab TLS 1.3 SAN iPAddress length 11–15 rejected (`sh23_crypto_tls13_peer_n11..n15_eval_smoke`); parametriserad längdsvepning 0–20 (`sh23_crypto_tls13_peer_nlen_sweep_eval_smoke`); leaf-cert `validity` notBefore/notAfter (`sh23_crypto_tls13_peer_dates_eval_smoke`); chain-count (`crypto_tls13_peer_chain`), serialNumber (`crypto_tls13_peer_serial`) och signatureAlgorithm (`crypto_tls13_peer_sigalg`); generaliserad SAN-validator (`crypto_tls13_peer_san_gen`) + konsoliderad ext-scanner (`crypto_tls13_hs_opt`). Cheap smokes importerar inte crypto. Nästa gate: rustls-peer pin-/trust-validering eller app-data (inte radera rustls). |
