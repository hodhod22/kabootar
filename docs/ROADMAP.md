...
| **Nästa kod** | Byta `http_fetch_async` till Kab-TLS (inte rustls). TLS 1.2 fixture-handshake samt PEM/Base64→strukturell X.509-DER är verifierade i Kab; nästa gate är X.509 issuer/signatur-trust-chain och live handshake i Kab. |
...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 Kab TLS application-data/AES-GCM unwrap med recordsäker raw-byte TCP-loopback + query Location (`sh23_crypto_http_fetch_eval_smoke`), deterministisk TLS 1.2 fixture-handshake (`sh23_crypto_tls_fixture_eval_smoke`) och strikt PEM/Base64→strukturell X.509-DER-gate för `TBSCertificate`/`signatureAlgorithm`/`signatureValue` (`sh23_crypto_root_eval_smoke`); rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** X.509 issuer/signatur-trust-chain och live handshake i Kab före byte av `http_fetch_async` från rustls. |
