...
| | |
|--|--|
| **Våg / steg** | **SH23** ✅ — krypto + TLS 1.2 **och TLS 1.3** i Kab (`cryptoHostDeleteOk` driven by `crypto_tls13_all.kab`; rustls finns kvar för icke-loopback HTTPS) |
| **Nästa kod** | SH23 färdigt på Kab-sidan: rustls-host finns kvar för icke-loopback HTTPS. Kab TLS 1.3 SAN iPAddress length 11–15 rejected (`sh23_crypto_tls13_peer_n11..n15_eval_smoke`); parametriserad längdsvepning 0–20 (`sh23_crypto_tls13_peer_nlen_sweep_eval_smoke`); leaf-cert `validity` notBefore/notAfter (`sh23_crypto_tls13_peer_dates_eval_smoke`); chain-count, chain-all, serial, sigAlg, pin, sammansatt trust-gate, X.509-struktur, AIA/OCSP, full handshake, TLS 1.2 high-level client (`crypto_tls12_client.kab`/`crypto_tls12_client_fetch.kab`), TLS 1.3 high-level client (`crypto_tls13_client.kab`/`crypto_tls13_client_get.kab`/`crypto_tls13_client_close.kab`/`crypto_tls13_client_fetch.kab`/`crypto_tls13_client_reconnect.kab`/`crypto_tls13_client_config.kab`/`crypto_tls13_client_post.kab`/`crypto_tls13_client_timeout.kab`/`crypto_tls13_client_generic.kab`/`crypto_tls13_all.kab`), scheme-match (`crypto_tls13_peer_scheme.kab`), CRLDP (`crypto_tls13_peer_crl.kab`), alert (`crypto_tls13_peer_alert.kab`), certreq (`crypto_tls13_peer_certreq.kab`), resume (`crypto_tls13_peer_resume.kab`) och no-rustls loopback (`crypto_tls13_client_loop.kab`); `httpFetch` kan nu nå rustls-peer via Kab-klient (`crypto_tls13_client_fetch.kab`/`crypto_tls13_client_generic.kab`); TLS 1.3 applikationsdata-GET (`sh23_crypto_tls13_peer_appdata_eval_smoke`); generaliserad SAN-validator (`crypto_tls13_peer_san_gen`) + konsoliderad ext-scanner (`crypto_tls13_hs_opt`). Cheap smokes importerar inte crypto. Nästa gate: SH28 arkivera `src/` när alla bootstrap/AOT/no-rustc gater passerar. |
...
- **Radera inte** `src/**/*.rs` medan Kabootar är ostabil. När en yta finns i `.kab`: **använd Kab**, inte Rust. När Kabootar är stabil: **arkivera** Rust och kör bara Kab.
...
**Klart när:** produkten körs **bara med `.kab`**. `src/` och `Cargo.toml` **arkiveras** (inte raderas i förtid) när Kabootar är stabil. En användare bygger och kör Kabootar **utan rustc**. Den sista process-starten är antingen (a) en **Kab-AOT-binär** producerad av Kab-JIT/AOT, eller (b) en **minimal oföränderlig stubb** som bara mmap:ar en image som *själv* är emitterad av Kab — och även den stubben ersätts (SH19/SH28). Inga “syscalls i Rust för alltid”.
...
| **SH17** | **JIT i Kabootar** — register-alloc + native-emit som `.kab` (ersätt Cranelift i `src/`). Får börja som subset (i64-loopar). Bred teknik: [Våg FT](#våg-ft--fart-alla-tekniker-i-kab) F7–F10 | Hot loop kompilerad av Kab-JIT; `src/` JIT-moduler raderas när smoke är grön | ✅ subset + **`jitFromOpsOk`** AccAdd-local/`add`/`const`/`load_local`/`store_local`/`ret` → inc+ret `os_mm_call` (`sh17_jit_from_ops_exec_smoke`); i64-loop-subset stängt; templates + `os_mm_*`; ingen ny Cranelift/RWX i Rust |
| **SH18** | **GC i Kabootar** — nursery/sweep som `.kab` (ersätt Rust-GC) | Frame-budget smoke utan `src/` GC; sedan radera | ✅ subset + **`gcAllocBurstOk`** + Kab-VM **`vNurseryCharge`** (`sh18_gc_alloc_exec_smoke`, `sh18_gc_vm_new_instance_exec_smoke`); bump/promote/sweep/barrier/TLAB/capstone; `gcHostDeleteOk` still false |
| **SH19** | **Laddare i Kab** — ingen `main.rs` som produkt. `.kab` startar runtime | `kabootar`-binär = Kab-bootstrap eller Kab-AOT | ✅ subset + **`loadEvalKbcRoundtripOk`** `.kbc` + **`loadEvalKbcbRoundtripOk`** packed v2 (`sh19_load_file_exec_smoke`, `sh19_load_kbcb_exec_smoke`); `loadMainDeleteOk` still false |
