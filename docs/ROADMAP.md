...
| **F18** | **App-CI** — tre kanoniska `.kab`-appar (same-room HTTP, 60 FPS game-loop, nd/AI-kernel) mot budget; inte bara `league_add_loop` | PR-gate: ingen tyst regression på de tre; mikro får inte vinna mot app | 📋 deepen: harness i `.kab` (inte ny `src/`-profiler) |
| **F19** | **Zero-copy same-room** — `sql()`-rad / nd-view → `http_request`-kropp utan stringify och utan extra `Value`-kopia | e2e: N rader lista utan JSON-hop (P17b deepen); alloc/request histogram | 📋 F15/P17a är “ingen stringify”; F19 är view/buffer |
| **F20** | **Typspecialisering** — JIT klonar het fn när IC alltid ser i64 eller f64; deopt vid ny typ | type-check borta i specialiserad loop; samma i64-gate som F8 | 📋 efter F7 exec + F2 IC i Kab-JIT; inte före |
| **F21** | **Lazy AOT** — AOT bara boot-graf (första ~100 ms / import-DAG); resten baseline-JIT + F8 | HTTP kallstart-budget (målordning: mät → kapa boot, inte 50 ms-löfte först) | 📋 F10/P13c deepen; ingen jätte-image som default |
| **F22** | **TLAB** — per-tråd nursery; promote till delad old när TLAB full | workers (F13) allokerar utan mutex per `new`; pause-histogram oförändrat | 📋 efter SH18 host-delete-path; meningslöst på en tråd |
| **F23** | **Mmap-bulk** — stora `array_f64`/`Uint8Array` via `os_mm` (fil eller anonym); samma pekare till GPU | science/ljud/bild utan extra copy; **inte** “större än RAM som default-heap” | 📋 P2/P16c/`os_mm` redan finns; bind i Kab-stdlib |
| **F24** | **TCO opt-in** — svansanrop återanvänder frame (`@tail` eller känd self-recursion i JIT) | djup rekursion utan stack-växt på markerade fn; stackspår bevaras default | 📋 F8 deepen; inte tyst TCO på all CALL (debug) |
...
**Nästa:** SH20–SH28 delete-gate deepen (alla gates fortfarande `false`); SH27 UI dual-bind deepen (nästa leaf efter ui_not: uiIsImplies). Se [Kabootar på egna fötter](#kabootar-på-egna-fötter--noll-rust). **Dok:** ROADMAP + `self_host/README.md` efter varje pass.
