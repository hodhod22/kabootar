...
| **Nästa kod** | Byta `http_fetch_async` till Kab-TLS (inte rustls). Kab har verifierad TLS 1.2 TCP-loopback för ClientHello/ServerHello/Certificate/ServerKeyExchange/CCS och AES-GCM-skyddad server-Finished samt PEM/Base64→X.509-DER. Nästa gate är verklig certifikatsignatur/trust-chain och live handshake i Kab. |
...
| **SH23** | **Krypto + TLS i Kab** — `import "crypto"` och trust/pinning i `.kab` (ersätt rustls-host) | HTTPS-smoke emitterad/verifierad i Kab | 🟡 Kab TLS application-data/AES-GCM unwrap med recordsäker raw-byte TCP-loopback + query Location (`sh23_crypto_http_fetch_eval_smoke`), verifierad TCP-loopback för ClientHello/ServerHello/Certificate, X25519-ServerKeyExchange och ChangeCipherSpec med längd-/algoritmkontroller och transcript (`sh23_crypto_tls_hs_eval_smoke`), deterministisk TLS 1.2 fixture-handshake (`sh23_crypto_tls_fixture_eval_smoke`) och strikt PEM/Base64→X.509-DER-gate för `TBSCertificate`/`signatureAlgorithm`/`signatureValue`, exakt DER för `sha256WithRSAEncryption`, v3-prefix, issuer/subject-RDN och `rsaEncryption`-SPKI med kanoniska modulus/exponent. SHA-256/PKCS#1-v1.5 kontrolleras även med en avgränsad 512-bitars RSA/e=3-fixture (`sh23_crypto_root_eval_smoke`); rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa:** krypterad Finished, verklig X.509-certifikatsignatur/trust-chain och live handshake i Kab före byte av `http_fetch_async` från rustls. |
