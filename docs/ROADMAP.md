...
**Aktivt fokus (2026-08):** P6b empty skip-list when self-host leaf compile is CI-fast; live OpenXR session / wasm XRSession.rAF. **P/GP0–GP5** + **GP6a–n subset** + **SC0–SC7** + **DX0–DX7** + SIM/DATA/IOT/APP MVP subset landad.
...
| **GP6n** | `game/xr` | **VR/AR-stöd** — headset present, tracked controllers, stereo cameras; WebXR/OpenXR via host FFI | ✅ subset (loader end-frame + XR rAF + compositor process/IPC + HMD present) |
...
**Checkpoint GP (nästa):** live OpenXR session + real xrEndFrame FFI call; wasm XRSession.rAF when browser grants session; P6b empty skip-list when leaf self-host <10s.  
**Checkpoint GP (landad):** OpenXR loader end-frame path + WebXR rAF bind/pump (`xrRunImmersiveRaf`); compositor process/immersive loop; P6 seed-only + P6b gate.  
