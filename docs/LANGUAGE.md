...
| 10 | Match + guards | ✅ host; self-host array/objekt/`n @ pat`/or/range/enum; **kab-only** const/`_` / `[x, y]` / `{ p, q }` / `1..=5` / `n @ 1..=5` / `1 | 2 | 3` / `(1 | 2)` / `..5` / `5..` / `[h, ...t]` / `{ k, ...s }` / `[h, ...mid, last]` / `if n != 3` / `Color.Red` / `Msg.Move(p)` / `xs @ [p, q]` / `wrap @ { k, ...s }` / `{ k: n @ 1..=5 }` / `[n @ 1, ...r]` / `Ok(n @ 1..=5)` / `Some(n @ 1..=5)` / `Option.Some(n)` / `Option<Number>.None` / `1.0..=2.0` | `match 1 { 1 => 2, _ => 0 }` → `jump_unless_const_eq`; **`match [1, 2] { [x, y] => x + y }`** → `jump_unless_array`; **`match { p, q }`** → `jump_unless_has_member`; **`match 5 { 1..=5 => 9 }`** / **`match 1.5 { 1.0..=2.0 => 9 }`** → `ge`/`le`; **`n @ 1..=5`** → DUP + `store_local` + range; **`match 2 { 1 | 2 | 3 => 9 }`** / **`(1 | 2)`** / **`1 | (2 | 3)`** → `emitMatchPatOr`; **`match 3 { ..5 => 1 }`** / **`5..`** → open `hasLo`/`hasHi`; **`[h, ...t]`** → `array_slice_rest`; **`{ k, ...s }`** → `object_rest`; **`[h, ...mid, last]`** / **`[h, ..., last]`** → `index_peek_from_end`; **`n @ 1..=5 if n != 3`** → `ne` + `jump_if_false`; **`Color.Red`** → `jump_unless_enum_variant`; **`Msg.Move(p)`** → `unpack_enum_fields`; **`xs @ [p, q]`** / **`xs @ [h, ...t]`** / **`wrap @ { k, ...s }`** → DUP + `store_local` + inner; **`{ k: n @ 1..=5 }`** → field + at + range; **`[n @ 1, ...r]`** → piece + at + rest; **`Ok(n @ 1..=5)`** / **`Some(n @ 1..=5)`** / **`Err(n @ 1..=5)`** → unwrap + at + range; **`Option.Some(n)`** / **`Option<Number>.None`** → `jump_unless_enum_variant` (`Option$Number` prefix) ([ROADMAP L6](ROADMAP.md#våg-l--language-systems-ready--subset)) |
| 10b | User `enum` | ✅ host + self-host unit/payload; **kab-only** `match Color.Red` / `Msg.Move(p)` / `Option.Some(n)` | `enum Color { Red, Green }`, `Color.Red`, `Msg.Move(x)` → `jump_unless_enum_variant` / `unpack_enum_fields`; **`enum Option<T>`** → `Option$Number` |
| 10c | `if let` / `while let` | ✅ host; self-host socker över `match`; **kab-only** `Some`/`Ok` / `1 | 2` | `if let Some(x) = Some(3)` → `jump_unless_option_some`; `while let Ok(v) = r` → `jump_unless_result_ok` + `break` på miss; **`if let 1 | 2 = x`** / **`while let 1 | 2 = r`** → `emitMatchPatOr` |
...
| 10k | Generisk enum `Option<T>` / `Result<T,E>` | ✅ host Option; self-host ctor, **två typparametrar**, `match`; **kab-only** `Option.Some(n)` / `Option<Number>.None` | `Option.Some(42)` → `Option$Number`; `Result<Number, String>.Ok(42)` ([GENERICS.md G9](GENERICS.md)) |
