...
| 10ac | `async fn` | ✅ host eval; self-host parse+emit `fn_async`; **kab-only** sync-kropp + `promise_resolve` | `async fn add(a, b) { return a + b }; await add(2, 3)` → **5** (`sh6_self_host_async_fn_ok`). Inte rust `schedule_bytecode_async` på Kab-VM. Inte `async =>`. Inte `async fn*`. |
