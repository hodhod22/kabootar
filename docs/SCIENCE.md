...
| `nd_max` / `nd_min` / `nd_argmax` / `nd_transpose` / `nd_swapaxes` / `nd_pad` / `nd_roll` / `nd_tensordot` / `nd_einsum` | Reductions + layout + contract + einsum subset (`ij,jk->ik`, `ij->ji`, `ii->`, `i,i->`, `ij,ij->`, `ij,ij->ij`, `i->`, `ij->i`, `ij->j`) |
| `mat_batch_qr` / `mat_batch_svd` / `mat_batch_solve` / `mat_batch_eig` | Batched decomps/solve/eig |
| `num_fftn` / `num_firwin` / `num_butter_biquad` / `num_polyphase_*` | n-D FFT + filter design + polyphase |
| `sparse_to_csc` / `sparse_from_csc` / `sparse_ilu0` / `sparse_icc0` | CSC + incomplete factorizations |
| `science/domain/pde` | 1D/2D heat/wave/poisson FD |
