...
| `fn*` / `yield` | generator functions | **`fn* gen() { yield 1; yield* [2, 3]; return 99 }`**, **`gen()`** → iterator, **`.next()`**; self-host + Kab-VM **`.next().value`** (`sh6_self_host_generator_yield_ok`); **`return 99`** completion (`sh6_self_host_generator_return_ok`); **`g.return(v)`** close (`sh6_self_host_generator_method_return_ok`); **`g.throw(e)`** resume `catch` (`sh6_self_host_generator_throw_catch_ok`); **`yield* [1, 2]`** (`sh6_self_host_yield_star_array_ok`); **`yield* inner()`** (`sh6_self_host_yield_star_generator_ok`); **`for x of gen()`** (`sh6_self_host_for_of_generator_ok`) |
