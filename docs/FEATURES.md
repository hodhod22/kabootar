...
| Rust | `if let` / `while let` | ✅ host; self-host socker över `match`; **kab-only** `if let Some(x) = Some(3)` / `while let Ok(v) = r` / `if let 1 | 2` / `while let 1 | 2` / `if let n @ Some(x)` |
