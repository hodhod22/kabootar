...
| Ursprung | Konstruktion | Status |
|----------|--------------|--------|
| Rust | `match` / mönstermatchning | ✅ host: tal, variabel, `_`, `Some`/`None`, `Ok`/`Err`, array/objekt, guards, enum. **Self-host:** samma + `if let`/`while let`; enum-IR (text-`.kbc` enum-register kvar) |
