...
6. ~~**SH19**~~ ✅ subset + deepen (`loadEvalKbcbRoundtripOk` packed `.kbcb` v2 eval; `loadKbcbHeaderOk`/`loadKbcbFixtureOk` i `load_validate.kab`; `loadMainDeleteOk` still false)
...
10. **SH23** 🟡 Kab TLS 1.3 SAN iPAddress length 11–15 rejected rustls P-256-peer `:28457` (`crypto_tls13_peer_n11..n15.kab`); length-4-adressvalidering via `crypto_tls13_peer_n4.kab`; parametriserad SAN-längdrejection via `crypto_tls13_peer_nlen.kab`; generaliserad SAN-validator (`crypto_tls13_peer_san_gen.kab`) + konsoliderad ext-scanner (`crypto_tls13_hs_opt.kab`) minskar duplicering; rustls-host kvar; `cryptoHostDeleteOk` false. **Nästa:** [Just nu](../docs/ROADMAP.md#just-nu).
