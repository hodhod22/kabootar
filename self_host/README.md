...
16. ~~**F10 AOT native-image policy**~~ ✅ (x64/arm64 ship/ret-op/nop-op load+verify/zero-op load+verify/one-op load+verify/add-op load+verify/sub-op load+verify/text+layout+native one+add+nop+ret/rodata/data + write/read bind); native machine-code deepen; `nollAotReady` still false
