...
12. ~~**SH25**~~ ✅ subset (`cliIsCompile` + `cliIsFmt` + `cliIsCheck` + `cliIsLint` + `cliIsVersion` + `cliIsHelp` + `cliIsDoc` + `cliIsBench` + `cliIsNew` + `cliIsInit` + `cliIsWatch` + `cliIsClean` + `cliIsAdd` + `cliIsRm` + `cliIsMod` + `cliIsLs` + `cliIsCat` + `cliIsServe` + `cliIsShell` + `cliIsNotebook`/`nb` + `cliIsRegistry` + `cliIsInstall` + `cliIsPublish`; kabtest KT8 `ktCliIsCoverage` + `ktCliHostDeleteOk`); radera `src/cli` deepen
