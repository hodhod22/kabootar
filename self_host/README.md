...
**Slutmål (nolltolerans):** hela stacken är `.kab`. Rust är skuld tills [SH28](../docs/ROADMAP.md#kabootar-på-egna-fötter--noll-rust) **arkiverar** den — **6 månader efter** att Kabootar stått helt på egna fötter och stabilt. Plan: [docs/ROADMAP.md — Kabootar på egna fötter](../docs/ROADMAP.md#kabootar-på-egna-fötter--noll-rust).
...
**Nolltolerans:** [docs/ROADMAP.md — strikt arbetsregel](../docs/ROADMAP.md#nolltolerans--strikt-arbetsregel). **Just nu:** [docs/ROADMAP.md — Just nu](../docs/ROADMAP.md#just-nu). Bekräfta SH-/F-/GP-/SC-steg innan kod. Ingen ny `src/**/*.rs`. Hoppa inte över SH5 → SH16 → SH6 → SH17 → … → SH28. Radera inte Rust förrän Kabootar stått helt på egna fötter och stabilt i 6 månader; när Kab-yta finns, använd den.
...
| Compile-DAG | **7** `.kab` (SH5 platå — `emit_sym`→`ast_defs`, `ownership`/`emit_exec`→`compile`, `parser_postfix`→`parser_exec`, `serialize_sections`→`compile`; gamla paths = `pub import`-facader); `vm_*` **&lt; 40** (SH6) |
...
| Fil | Roll |
|-----|------|
| `compile.kab` | `compile(source)` / `compileIr` + emit-session (`eTrampFn` — kolliderade med parserns `tramp`) + O5 `checkOwnership`/`checkSource` (`@manual` via `guardAndPreprocess`) |
| `ast_defs.kab` | AST-consts + node-helpers + emit (sym/op/fn/kropp-orkestrering) — `emit_sym` ihopslagen hit |
| `parse.kab` | `parse` = tokenizeExec + parseTokensExec |
| `lexer.kab` / `parser.kab` / `emit.kab` / `serialize.kab` | tunna `pub let`-facader |
| `emit_sym.kab` / `emit_exec.kab` / `ownership.kab` | tunna `pub import`-facader (bakåtkompat) |
| `parser_exec.kab` | per-call session + tramp |
...
cargo test --test sh18_shared_obj_mut_smoke -- --test-threads=1
...
4. **SH5 platå** — compile-DAG **7**: `emit_sym`→`ast_defs`, `ownership`/`emit_exec`→`compile`, `parser_postfix`→`parser_exec`, `serialize_sections`→`compile`; gamla paths = `pub import`-facader. Fallgrop fixad: compile-lokal `tramp` → `eTrampFn` (kolliderade med parserns privata `tramp(sess)` — `E["tramp"]`-nyckeln orörd). Merger 4–5 valda via `bootPolicy("maxBytes")` = 64 KiB: `parser_exec`+`postfix` ≈48 KiB och `compile`+`serialize_sections` ≈47 KiB — `parser_stmt`/`emit_*_body`-merger överskrider (emit_expr_body är redan 72 KiB). **`match`**: enum unit + payload + `if let`/`while let`. Tidigare känd regression (SH18 WIP) — Kab-VM `Cannot add Undefined and Number(1)` i stora suiter — är **löst** i deepen 9 nedan.
5. ~~**SH17/SH18**~~ ✅ subset + deepen (`jitMmapOk` mmap/exec dual-bind; loop8/loopN/arith-imm/bit-ops/shifts/unary/eq/ne/lt/gt/le/ge/test/je/jne/jmp/jl/jle/jg/jge/nop `os_mm_call`; `jit_from_ops` breddad i64-op-legalitet + ärlig `jitCanCompile`, `sh17_jit_ops_wide_exec_smoke`; `gcHostDeleteOk` host-GC dual-bind) + **SH18 deepen-löv** (`gc_tri` tricolor, `gc_incr` barnskuggad worklist `gcIncrScanSlice`/`gcIncrTreeMarkOk`, `gc_rem` remembered set, `gc_copy` semi-space, `gc_sweep` free-list bytes, `gc_manual` @manual-exempt → `gc_deep`/`gcChainDeepOk`/`gcCapstoneDeepOk`) + deep-2 (`gc_age` tenuring `gcAgeCyclesOk`, `gc_card` card-scan `gcCardScanOk`, `gc_safe` safepoint-polls `gcSafeRunOk` → `gcDeep2Ok`/`gcChainDeep2Ok`/`gcCapstoneDeep2Ok`) + deep-3 (`gc_compact` sliding `gcCompactFwd`/`gcCompactOk`, `gc_los` large-object-space `gcLosSpace`/`gcLosOk`, `gc_fl` free-list `gcFlFit`/`gcFlAlloc`/`gcFlAllocOk` → `gcDeep3Ok`/`gcChainDeep3Ok`/`gcCapstoneDeep3Ok`) + deep-4 (`gc_roots` shadow-stack `gcRootsScanOk`, `gc_pace` heap-growth `gcPaceGrow`/`gcPaceOk`, `gc_weak` weak refs `gcWeakGet`/`gcWeakSweep`/`gcWeakOk` → `gcDeep4Ok`/`gcChainDeep4Ok`/`gcCapstoneDeep4Ok`) + deep-5 (`gc_final` finalizer-kö `gcFinalQueue`/`gcFinalOk`, `gc_ephemeron` fixpoint `gcEphMark`/`gcEphOk`, `gc_mature` pretenuring `gcMatureDecide`/`gcMatureOk`, `gc_satb` SATB-barriär `gcSatbLog`/`gcSatbDrain`/`gcSatbOk` → `gcDeep5Ok`/`gcChainDeep5Ok`/`gcCapstoneDeep5Ok`) + VM-audit (`gc_vm_audit` leaf `gcVmRoots`/`gcVmAuditOk`; `vGcRootsAuditS` i `vm_s_stack.kab` körs vid nursery-collect — räknar roots/live/dead + `gcDeadBytes` i `S["insts"]` med O(1) sträng-nycklad seen-map, enkelväders-roots (`lastThis`/`result`/`classes`/`_args`) + transitiv BFS-mark (`gcMarkedN`) + riktig sweep (`vGcSweepDeadS` tombstones döda slots, `gcReclaimedN`/`gcReclaimedBytes`) + inkrementell mark i slices (`vGcCollectBeginS`/`vGcMarkSliceS`/`vGcFinishMarkS`, `vGcMarkSliceN`=64/charge; allocated-black vid fresh publish under mark; `gcUsed`→live-bytes + `gcPaceGrow` på cap) + write-barrier (`vGcLogWriteS`, `gcWriteN` vid collect) + id-freelist (`vGcTakeInstIdS` återanvänder sweepade slots på alla alloc-siter) + collect-mätning (`gcCollectMs`/`gcMarkFieldsN`/`gcOverBudget`) + readiness-gate (`gcHostReadyOk`→`gcHostReady` per cykel); delas av `new_instance` och `vPublishGenS` (generator-publish laddar `gcInstShellBytes`), charge-täckning via `vHeapPutFreshS`; leaf-paritet `gcVmTransitive`/`gcVmMarkMap`; obs: `__kab_*`-keys är read-only för guest-`.kab`) + promote-boundary fix (`gcPromote >=`; `used == cap` tömmer nursery, `gcLoadCycleOk` kör `(0, cap, live)`) + deep-6 (`gc_lazy` lazy-sweep-slices, `gc_idle` idle-mark, `gc_minor` nursery-only via rem-set → `gcDeep6Ok`/`gcChainDeep6Ok`/`gcCapstoneDeep6Ok`) + **lazy sweep i Kab-VM** (`vGcFinishMarkS` armerar `gcSweepActive`/`gcSweepAt`/`gcSweepSeen`/`gcSweepMinNew`; `vGcSweepSliceS`=64 slots/charge tombstonar + weak-clear per slot + freelist-återfyllning; `vGcTakeInstIdS` låser freelist under sweep — fresh = id ≥ watermark; settle vid heap-end: `gcUsed`/`gcCap`/`gcHostReady`/`gcCollectMs`; `vGcRootsAuditS` dränerar mark+sweep STW; `vGcWeakClearObjS` per-slot weak-clear) + deep-7 (**generationer i Kab-VM**: minor collect — unga id ≥ `gcOldId`, seed unga roots + rem-set `gcRemIds`/`gcRemSeen` via `vGcLogWriteS` old→young; major vart 4:e `vGcMajorOk` — promotion `gcOldId`/`gcOldBytes` + rem-prune; `vGcAllocBlackS` på alla publish-vägar; `vGcIdleS` idle-drain; nursery-charge på literal-ops make_array/make_object/concat/make_ok/err/some/none/arrow) + deep-8 (survival-pacing `vGcMajorSurvPct`/`gcMajorDue` — hög minor-överlevnad → major; barriär-/rot-täckning: `vMergeInstInPlace` skjuter `vGcLogWriteS` per fält, `g["closeRet"]` loggas, `curGen`+`genClose`-värden är roots; gap: nästlade plain-objekt i fält/roots skannas inte — ingen objekt-identitet i Kab) + **deep-9** (regressionen `Cannot add Undefined and Number(1)` löst: collect armeras via `gcCollectPending` och börjar vid op-gräns i `runOpsStepS` — inte mitt i en op där operanderna bara levde i dispatch-locals; `S["localGMap"]` local→global-idx för modul-framen eftersom modul-`let` bor i både `localNames` och `globals` (host env-paritet: main `store_local`, fn `acc_add_global`) — `vLocalGetS`/`vLocalSetS` läser/skriver genom `S["globals"]`, kartan rensas i fn-prep och följer med sparade frames; `S["insts"]` id-nycklad **map** istf id-indexerad array (dödar O(`__kab_oid`)-padding per publish); rekursiv root-scan — nästlade arrayer, fält på o-publicerade wrappers, `lastMemObj`/`savedFrames`/`_args`-kedjor/`yieldVal`/`ysdel`; lazy-sweep över `gcSweepKeys`-snapshot + freelist-prune vid settle; heap-resident `gcRootsN`/`gcMarkedN` så `marked==live` håller; `self_host_ownership_suite`+`self_host_parser_suite` gröna, 93/93 `sh18_`; `gcHostDeleteOk` fortfarande `false` by design) + **test-race-fix** (`KAB_VM_EXEC_ACTIVE` global→`thread_local` — parallella evals nollade varandras "inne i Kab-VM"-markering → nästlad import re-entrerade VM:n → tomma exports "Undefined variable"/"unsupported opcode"; + `write_atomic` temp+rename för `.kbc`/`.kbcb`/seed-dag-cachewrites — mmap-läsare ser aldrig halvskrivna bilder) + **deep-10** (h-keyed minor-täckning: oid-hållare `{__kab_oid}` utan `vmI` är old-space — `vGcLogWriteS` bokar rem-set under `"h"+oid` även när oid ≥ `gcOldId` (generationsjämförelsen gäller bara riktiga `vmI`), `vGcSweepSliceS` tombstonar aldrig oid-slots i minors (nåbara via otracerade gamla inster), write-log `"h"+oid` under aktiv mark; täckt i `sh18_gc_vm_minor_smoke`) + **SH17 PIC i riktig loop** (`vm_run_jit.kab`: `vJitClassifyS` cachar `vClassifyCallee`-records per call-site i `S["jitSites"][" "+ip]` — shape `f`/`a`/`j`/`m`/`gn`/`gr`/`gt`, mono → PIC ≤4 → megamorf; inkopplad i `runOpCallNS`/`call_from_array`/`vHostCallVmValue`; `{vmJit:1, jitOps|jitSlots+jitSlotOps}`-callees exec:ar fused lowering via `vJitExecOk` med värde-spegeln `jit_from_ops_any_val`; gates `sh17_jit_pic_ops_loop_smoke` — vanligt programs `call`-op når jit-pathen — + `sh17_jit_pic_loop_smoke` maskineri)
6. ~~**SH19**~~ ✅ subset + deepen (`loadEvalKbcbRoundtripOk` packed `.kbcb` v2 eval; `loadEvalKabRoundtripOk` `.kab`-källa via `load_src.kab`; `loadArgvKind` argv-dispatch via SH25-gates i `load_argv.kab`; `loadKbcbHeaderOk`/`loadKbcbFixtureOk` i `load_validate.kab`; `loadMainDeleteOk` still false; 56/56 `sh19_` gröna). Sanerat: 10 korrupta `load_*`-shards hade inbäddade `NN|`-radnummer; `cryptoHostDeleteOk` kräver `KAB_TLS_PEERS=1` så gate failar stängd istf krasch (native-`Err` är ofångbar — `catch` tar bara `throw`-värden, medveten semantik). **Akutundantag:** `src/main.rs` kör CLI på 64 MiB-stack-tråd (self-host-compile av `try`+call overflowade ~1 MB main-stack) — återgång: återställ `main.rs` om emit-rekursionen grundas
7. ~~**SH20**~~ ✅ subset (JSON/datum/regex + math + objekt + collections + colget + collen + colpush + colpop + colfirst + collast + colrest + colempty + colconcat + colrev + colcontains + colindex + colcount + coltake + coldrop + colzip + colunzip + colflat + colunique + coleq + colclone + colrepeat + colfill + colrange + colsum + colmax + colmin + colprod + colavg + colmed + colmode + colsort + coldesc + colfind + colrfind + colrix + colslice + colwin + colchunk + colrot + colpad + colilv + coltr + coldiag + colident + coltrc + colrow + colcol + colshape + colrshp + coldot + colmv + colmm + colout + colcrs + coldet + colnorm + colunit + colproj + colrej + coldist + collerp + colscale + coladd + colsub + colmul + coldiv + colneg + colabs + colsign + colclamp + colmod + colpow + colsqrt + colsqr + colcub + colfloor + colceil + colround + coltrunc + collog + collog2 + collog10 + colexp + colsin + colcos + coltan + colasin + colacos + colatan + colatan2 + colhypot + colcbrt + colimul + colclz32 + colfround + colf16round + colsumprec + collog1p + colexpm1 + colsinh + colcosh + coltanh + colasinh + colacosh + colatanh + colfmod + colrandom + colpi + cole + colln2 + colln10 + collog2e + collog10e + colsqrt2 + colsqrt12 leaves) + **opt-pass:** merge-sort (`colsort`/`colmed`/`coldesc`, O(n²)→O(n log n) VM-ops), map-räkning (`colunique`/`colmode`, O(n²)→O(n) med `stdColContains`/`stdColCount`-fallback för icke-hashbara + `x == x` NaN-gate), JSON-codec run-slice via `str_index_of` + `array_join` + `object_array_push`-builders (in-place; bare `push()` kopierar hela arrayen per anrop i Kab-VM); bench `_bench_sh20.kab`: sort-1500 >10 min→~5.8 s, JSON-21KB parse 62→53 s — väggen är Kab-VM per-op-kostnad (SH17 JIT); radera natives deepen
...
13. ~~**SH26**~~ ✅ subset (`sciNdLenOk` + `sciFftPow2` + `sciSub` + `sciDiv` + `sciNeg` + `sciAbs` + `sciMax` + `sciMin` + `sciClamp` + `sciPow` + `sciSqr` + `sciCub` + `sciSign` + `sciMod` + `sciSqrt` + `sciFloor` + `sciCeil` + `sciRound` + `sciTrunc` + `sciExp` + `sciLog` + `sciSin` + `sciCos` + `sciTan` + `sciNdAdd` + `sciNdSub` + `sciNdMul` + `sciNdDiv` + `sciNdDot` + `sciNdSum` + `sciNdMatMul` + `sciGpuKernelOk`); native GPU/conv2d/fft-kernel deepen
...
24. **SH6 oid-fix** — repeated `new` + class-field init → `vm stack underflow` (pre-existing, e1e88583-era — **ej** SH18-regression): instansliteralen `"__kab_oid": id` (vmI) delade namnrymd med host-`NEXT_OID`, så `writeback_object_args` oid-match kunde mergea en stale instans-CoW-klon in i levande `S` → op-replay på gammal stack. Fix: ingen explicit `__kab_oid` på instanser (`vmI` kvar som heap-id, unikt host-oid auto-stamplas) + `writeback_object_args` matchar `Rc::ptr_eq` först, oid som fallback. Gate `sh6_repeated_new_instance_field_init_ok`; 93/93 `sh18_` + 245/245 `sh20_` gröna. Kända orörda (pre-existing, separat pass): `delete`/`computed_delete`/`for_of_generator`. Kvar gap: ~~heap-nycklar `" "+id` i `vm_s_stack.kab` delar vmI↔oid-rymd~~ **löst** — `vInstKeyS` namespacer: `" "+vmI` för instanser, `"h"+__kab_oid` för host-oid-objekt (`make_object` stämplar `__kab_oid` från host-`NEXT_OID`, sessionens `vmI` börjar på 1 → numerisk kollision; en churn-inst med `vmI == oid` skrev tidigare över hållarens heap-slot). Alla heap-lookup/publish/mark/sweep/writeback/rem-set/write-log går via `vInstKeyS`; `vWritebackAfterMethodS` stämplar `__kab_oid` (inte `vmI`) på oid-mottagare. Gate `sh18_gc_vm_plain_exec_smoke`; 500-instans stress ~1.35 s/instans = trampolin-kostnad (SH17-territorium), inte fail.
25. **SH6 import-bindingsfix** — toppnivå-`import` från host-eval (`eval_source`, REPL/tester) band filmoduler via Kab-VM, som håller globals/exports i egen session `S` → importerarens env fick inga bindings/`exported_names` → `Undefined variable` (t.ex. `g7_mobile_shell_chrome_back_tabs`). Fix: `eval_program_for_import` markerar `KAB_VM_EXEC_ACTIVE` runt modul-eval i `eval_file_module` — samma host-engine-undantag nästlade loads redan tar — så `run_module` fyller env + export-marks. Gate `sh6_import_binds_exports_ok`. Huvudprogrammet (`run_file`/`eval_file_cached`) kör fortfarande Kab-VM.
26. **kstyle objekt-iterationsfix** — `lib/kstyle/parse.kab` `resolveVars` loopade `while i < len(vars)` + `vars[i]` över ett plain objekt. I Kab-VM räknar `len({})` det gömda `__kab_oid`-fältet (=1), så `vars[0]` gav `Invalid index access on {__kab_oid}` (`h6c_browser_chrome_smoke`). Itererar nu `object_keys(vars)` och skippar `__oid`/`__kab_oid`. Gate `kstyle_kabootar_parse_resolves_vars`. Notera: Kab-VM-semantik — `len(obj)` inkluderar oid-fält; objekt ska itereras via `object_keys`, aldrig numeriskt.
27. **SH27 session persist** — `lib/kbrowser/session.kab`: tab/session-state skrivs som JSON till `/session/tabs` via `kos/vfs` (`saveSession`/`loadSession`/`resumeSession`/`clearSession`/`sessionPersistOk`). `nav.kab` write-through på varje mutation (`navGo`/`navBack`/`navForward`/`navOpenTab`/`navCloseTab`/`navSwitchTab`) och `bootNav` återställer persist session istället för alltid-färsk `createSession`. Gates `sh27_session_persist_in_kab`/`sh27_session_smoke_in_kab` + `sh27_session_persist_smoke` (`examples/sh27_session_persist_smoke.kab`). VFS är per-process in-memory — persist gäller inom boot/shell-sessionen; host-mount/snapshot ger disk-persist på kOS.
28. **SH27 delete-gate-prep** — `examples/sh27_delete_gate_smoke.kab` kör hela rundan öppna → navigera → back/forward → stäng flik via `kbrowser/nav` + `history` + `session`, utan chrome-natives och utan chrome-mount (`kb_navigate` = bara load/paint). Gate `sh27_delete_gate_in_kab` + `sh27_delete_gate_smoke`. `uiHostDeleteOk()` stannar `false` tills host-chrome faktiskt är ersättbart.
29. **SH16 @version-compile-fix** — `compile_source_self_host` skickade rå källtext till self-host-`compile`; header-direktiv (`@version "1.0.0"`, `@manual`, `@gc`, …) är toolchain-metadata, inte språk, så lexern kraschade `{"type":"@"}` rad 1 → `SH16: rust compile fallback disabled` på cache-miss (ren checkout; maskerades av gitignored `.kabootar/cache`). Fix: `scan_header_directives` + `strip_header_directives` + `strip_version_directive` före self-host-lexern — bara direktivrader tas bort (ingen comptime/html/actor-expansion: källor utan direktiv når compilern verbatim), `memory_mode` från meta. Gate `self_host_compile_strips_version_directive`; `pagination`/`kstyle/parser`/`kdom/paint` kompilerar nu kallt.
30. **SH27 host-inventering + theme-brygga** — inventerade `src/runtime/kabootar_browser/`: Kab täcker chrome-policyn (`nav`/`history`/`session`, `theme::homePage`, `load_policy::titleFromUrl`), men `applyBrowserTheme` nådde aldrig `inner.stylesheet` — CSS strandade i `__kstyle`, så Rust `default_chrome_theme_css` var fortfarande den enda som nådde `paint()`. `theme.kab` anropar nu `kb_theme(css)` efter `kss(css)`. Kvar i Rust (medvetet): render-slot-spegel av `tabs`, `default_home_document`-fallback, `title_from_url` render-state + äkta host-capability (load_page, paint, viewport/safe_area, input, kv8). Inventeringstabell i [lib/kbrowser/ROADMAP.md](../lib/kbrowser/ROADMAP.md); `uiHostDeleteOk` stannar `false`. Gate `sh27_theme_bridge_in_kab`.
31. **SH27 KB-H2: load-policy på produktvägen** — `host_nav.rs::effective_mode` var bara en spegel (kommentar: "Kab is product policy source") men `nav.kab` konsulterade aldrig `load_policy`; `kb_navigate`→`load_page` valde mode själv. Nu äger `nav.kab` preferensen (`navModePref`, default `"auto"`, pin via `navSetMode`) och kör `navApplyMode` → `kb_set_os_mode(effectiveMode(url, pref))` före varje `kb_navigate` — Kabs beslut når host fetch-mode på riktiga navvägen; Rust-mirror + dess Auto-retry är kvar som fallback. `loadPlan(url)` i `load_policy.kab` buntar mode+kind+title+virtualHome; `loadPolicyOk` täcker den. Gates `sh27_load_policy_in_kab` + `sh27_load_policy_smoke` (`examples/sh27_load_policy_smoke.kab`).
32. **G11/G7 stale-smokes reparerade** — `kbrowser_kos_smoke`/`kbrowser_native_smoke`/`kbrowser_wasm_smoke`/`kbrowser_mobile_smoke`/`kbrowser_mobile_shell_smoke` kraschade alla med `vm unsupported opcode in main` (dot-access `sync.mode`/`vp.orientation` + bare-expr + `print` på toppnivå — Kab-VM main-program har smal opcode-mängd) och importerade aldrig `lib/kbrowser`. Nu kör alla via Kab-ytan: `core` (`navigate`/`sync`/`info`/`paint`), `nav` (`navUrl`), `mobile_chrome` (`applyPhoneViewport`/`applySafeArea`/`go`/`goBack`/`listTabs`); medlemsaccess i fn eller `obj["key"]`, `println` + `return`. Manuella `kb_set_os_mode("kabootar")` borttagna — `navApplyMode` auto-väljer mode per URL.
33. **SH27 game-loop i Kab** — `lib/kab/ui/ui_loop.kab`: `uiLoopCreate`/`uiFrameBegin`/`uiFrameEnd`/`uiLoopOk` — caller-`while` + begin/end-par, dt från `Date_now`, pacing via `sleep_ms(budget - cost)`, miss-räkning via `uiFpsOk`. Host = bara clock/yield/input/surface/present. Två Kab-VM-begränsningar hittades (pre-existing, även `game_playable_2d.kab` föll): fn-värden (`{vmFn: N}`) kunde inte anropas via host `call_value` — **fixat i 34** (callback-brygga) — loopen behåller ändå explicit `while`+begin/end som billigaste formen; `ctx.fillRect(...)` receiver-sugar saknades då — **fixat i 35** — smoken använder nu dot-form (`ctx["fillRect"](ctx, …)` fortfarande giltig, bracket är raw som host `read_index`). `missed` är informativt på tolkade VM:n (native-bridge per frame > 16ms). Gate `sh27_game_loop_in_kab` + `sh27_game_loop_smoke` (`examples/sh27_game_loop_smoke.kab`).
36. **SH27 produkt-hooks: Rust-produktrester ersatta** — `BrowserInner.tabs`/`active`/`next_id` (render-slot-spegel av Kab-sessionen) bort → en enda `tab: BrowserTab`; döda `tab.title`-fältet bort (skrevs aldrig läst — window-titlar är separata, session-titlar lever i `history.kab` som nu sätter riktiga `tab["title"]` via `titleFromUrl`). `default_home_document`/`title_from_url` finns kvar men bara som bootstrap/host-default: `nav.kab` installerar `kb_set_home_provider(theme.homePage)` + `kb_set_title_provider(load_policy.titleFromUrl)` vid import — `load_page` konsulterar providerna via `call_value` (Kab-DOM live-resolved ur registret). Lagade dessutom en pre-existing env-split: varje `create_global_env` fick ett eget `KabootarBrowser`-Arc → `kb_theme`/`kb_set_*` i modul-env nådde en annan browser än `kb_paint` i main-env; `SHARED_BROWSER` thread-local gör browsern en runtime-singleton per tråd (som DOM-registret/event-kön). Kvar i Rust = render-slot + äkta host-capability (load/paint/viewport/input/kv8); `uiHostDeleteOk` stannar `false` tills en Kab-renderväg ersätter även dem. Gates `sh27_product_hooks_in_kab` + `sh27_delete_gate_smoke` (assertar `kb_product_hooks()` + renderad `theme.homePage`).
37. **sleep_sync: verklig pacing på tolkade VM:n** — `sleep_ms(n)` är async by design (promise → `WakeAt::WallMs`, resolvas exakt av `drain_scheduler_step` vid await/drain) men en bar `sleep_ms(n)` i synk kod är fire-and-forget — den pacar aldrig. Det gamla "~280ms för 5ms"-fyndet var delvis mätarbrus (native-anrop via Kab-VM kostar ~0.5–1s vardera i debug-build) men pacing-felet var reellt: `uiFrameEnd` schemalade bara en promise ingen drainade. Ny `sleep_sync`-native = direkt `thread::sleep(ms)`; `ui_loop` pacar nu med `sleep_sync(budget - cost)`. `sleep_ms` oförändrad för `await`-semantik. Gate `sh27_sleep_sync_pacing`.
34. **SH27 vmFn callback-brygga** — host `call_value` kunde inte anropa Kab-VM fn-värden (`{vmFn: N}`/`{vmArrow}`) → modul-callbacks (rAF/`game_tick`/`fixedTick`/timer/promise) failade med `Not a function`. Fix: thread-local bridge-stack `KAB_VM_CALL_BRIDGE` i `src/bytecode/vm.rs`; `eval_kbc*_via_kab_vm` pushar vm-imagens `kabVmHostCallValue`-export medan sessionen lever; ny `call_value`-arm för marker-objekt anropar bron med `[callee, args]`. Kab-sidan: `vModEnterB` stashar `bcFns`/`bcPool`/`bcGlobals` i `S` (sparas/restoras i `vSaveModuleFrame`/`vRestoreModData`), `vHostCallVmValue` (exec_core, re-export via alias — ingen wrappande fn) klassificerar via `vClassifyCallee` och applicerar via `vApplyCalleeS` i live-sessionen. Buggen "arrow-body med captured-var-write tappar ytterrens resultat" var **inte** VM:n — den var dubbel sess-clobber: parsern (`sess["pBindSym"]` överskrevs av inner-assign i arrow-bodyn → `let h` fick `sym`="y") och emittern (`E["eAssignSym"]` clobbad av nested emit → `store_global` på fel slot). Båda fixade via hoistade lokaler (konvention 52/53). Gates `sh27_closure_capture_let_sym` + `self_host_parser_suite`. Gates `sh27_vm_fn_callback_bridge` + `sh27_vm_fn_callback_smoke` (`examples/vm_fn_callback_smoke.kab`).
35. **SH27 receiver-sugar på native-metodobjekt** — host `get_member_value` wrappar `NativeFunction`-fält i `BoundNative` för `__kab_ctx`/`__kab_gl_ctx`/`__kab_gl_tex`/`__kab_host_canvas`/`__kab_game_surface`/`__kab_mo`-objekt (`maybe_bind_native_method`), men Kab-VM `vMemberGetS` returnerade råfältet → `ctx.fillRect(...)` kallade natives utan receiver. Nu: `vMemberGetS` wrappar native-fält på markerade objekt i `{vmHostM: fn, vmR: obj}` (ny sentinel — inte `{vmM}` som går via `vCallRunMeth`-trampen); `vClassifyMethHost` → `kind: "hostMethod"`; `vApplyMethHostS` → `vCallHost(meth, [recv]+args)`. `bytecode_host_is_native` (ny host-native) speglar host-semantiken: bara `NativeFunction` binds, AST-`Function`-fält lämnas råa. Bracket `ctx["fillRect"]` fortsätter rå (host `read_index` binder inte heller) — `ctx["fillRect"](ctx, …)` funkar fortfarande. `{vmHostM}` tillagd i `kab_vm_callable_marker` så sentinelen även klarar host-bryggan. OBS: `.kabootar/cache` kan hålla inälvtade .kbc-artifakter som maskerar ändringar — radera cache vid mystiska "expected number"/förfel vid utveckling. Gates `sh27_host_method_receiver_sugar` + `sh27_host_method_sugar_smoke` (`examples/vm_host_method_sugar_smoke.kab`); `sh27_game_loop_smoke` använder nu dot-form.
