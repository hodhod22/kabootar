...
**SH6 = KLAR.** **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** packed `.kbcb`. **SH20** JSON codec. **SH21** OS write/read. **SH22:** `sqlExecOk` (`sh22_sql_exec_smoke`). **SH23:** SHA-256 FIPS + HMAC-SHA256 RFC 4231 + TLS 1.2 P_SHA256 PRF/Finished eval (`sh23_crypto_sha_eval_smoke`, `sh23_crypto_hmac_eval_smoke`, `sh23_crypto_tls_prf_eval_smoke`). rustls-host kvar (`cryptoHostDeleteOk` false). **Nästa: handshake-records/cert-verify i Kab, därefter rustls-delete.** Inte fler crypto-flag-kloner eller sqlIs*-kloner.
...
10. **SH23** 🟡 SHA-256 FIPS + HMAC-SHA256 RFC 4231 + TLS 1.2 P_SHA256 PRF/Finished eval (`sh23_crypto_sha_eval_smoke`, `sh23_crypto_hmac_eval_smoke`, `sh23_crypto_tls_prf_eval_smoke`); rustls-host kvar. **Nästa:** handshake-records/cert-verify i Kab; rustls-delete deepen
