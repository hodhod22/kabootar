...
**SH6 = KLAR.** **SH17** i64-loop-subset stängt. **SH18** nursery på `new_instance`. **SH19** packed `.kbcb`. **SH20** JSON codec. **SH21** OS write/read. **SH22:** `sqlExecOk` (`sh22_sql_exec_smoke`). **SH23:** policy-gates pekar nu på rätt `lib/kab/crypto/`-blad; shift-parsern har korrekt `op`/vänster/höger-init (`sh23_self_host_shift_high_word_ok`) och emittern ger `bit_or` (`sh23_self_host_shift_or_emits_bit_or`). **Nästa: körningens stackväg, sedan SHA-256-eval i Kab.** Inte fler crypto-flag-kloner eller sqlIs*-kloner.
...
10. **SH23** 🟡 policy-subset (`cryptoTls12Ok` + `cryptoRootPem` + `cryptoTls13Ok` + `cryptoSha256Ok` + `cryptoHmacOk` + `cryptoAes256Ok` + `cryptoChaChaOk` + `cryptoGcmOk` + `cryptoEd25519Ok` + `cryptoX25519Ok`); rätt bladvägar verifieras. **Nästa:** SHA-256-eval i Kab; rustls-delete deepen
