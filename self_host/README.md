...
| Compile-DAG | **9** `.kab` (SH5 platå — `emit_sym`→`ast_defs`, `ownership`/`emit_exec`→`compile`; gamla paths = `pub import`-facader); `vm_*` **&lt; 40** (SH6) |
...
4. **SH5 platå** — compile-DAG **9**: `emit_sym`→`ast_defs`, `ownership`/`emit_exec`→`compile`; gamla paths = `pub import`-facader. Fallgrop fixad: compile-lokal `tramp` → `eTrampFn` (kolliderade med parserns privata `tramp(sess)` — `E["tramp"]`-nyckeln orörd). Inte `parser_stmt`/`postfix`/`emit_*_body` förrän leaf ≤10 s / ~550 rader. **`match`**: enum unit + payload + `if let`/`while let`. Tidigare känd regression (SH18 WIP) — Kab-VM `Cannot add Undefined and Number(1)` i stora suiter — är **löst** i deepen 9 nedan.
