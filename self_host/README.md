...
| Compile-DAG | **8** `.kab` (SH5 platå — `emit_sym`→`ast_defs`, `ownership`/`emit_exec`→`compile`, `parser_postfix`→`parser_exec`; gamla paths = `pub import`-facader); `vm_*` **&lt; 40** (SH6) |
...
4. **SH5 platå** — compile-DAG **8**: `emit_sym`→`ast_defs`, `ownership`/`emit_exec`→`compile`, `parser_postfix`→`parser_exec`; gamla paths = `pub import`-facader. Fallgrop fixad: compile-lokal `tramp` → `eTrampFn` (kolliderade med parserns privata `tramp(sess)` — `E["tramp"]`-nyckeln orörd). Merge 4 valdes via `bootPolicy("maxBytes")` = 64 KiB: `parser_exec`+`postfix` ≈48 KiB är enda par under taket — `parser_stmt`/`emit_*_body`-merger överskrider (emit_expr_body är redan 72 KiB). **`match`**: enum unit + payload + `if let`/`while let`. Tidigare känd regression (SH18 WIP) — Kab-VM `Cannot add Undefined and Number(1)` i stora suiter — är **löst** i deepen 9 nedan.
