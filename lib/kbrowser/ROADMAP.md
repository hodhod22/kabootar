...
|| **KB-H5** | KSS-parse i Kab på produktvägen | ✅ `kv8Page` returnerar `sheet` = `kstyle/parse.parseSheet(css)` på provider-kontraktet (`:root`-vars, `var()`, `@media` löses i Kab); hosten konverterar värdet via `kstyle::stylesheet_from_value` — ingen css-relex i `load_page`/`paint`/`run_kv8_script` (`sh27_kstyle_parse_in_kab`, `sh27_kstyle_parse_smoke`). Rust `parse_stylesheet` kvar bara för host-capability/fallback: `kv8_css(ctx,…)`/`kv8_run_ui`, `vfs_module`, bootstrap-chrome-tema |
|| **KB-H6** | kv8-eval i Kab på produktvägen (inkrementell) | ✅ första snittet — `kb_set_eval_provider(kv8/run.kv8RunPage)`: `kv8CanEval` vandrar AST:t mot eval-kuvertet (kinds+ops) och declinar *innan* någon DOM-mutation → Rust `eval_script`-fallback orörd; Kab-evaln muterar live-registret via `kdom_*` och hosten drar in det via `live_resolve_deep` i render-sloten. `evalRoute["kab"]` bevisar routen (`sh27_kv8_eval_in_kab`, `sh27_kv8_eval_smoke`). Kvar i Rust: opt/JIT/promise/context-paritet + allt utanför kuvertet |
