...
**Gates:** `evalSource(source)` / `evalSourceKab(source)` — Kabootar interpreter (`evalSourceWith`) only. **H6a:** `evalSourceRust` / `kv8_eval_source` deleted.

Literals, ident (incl. `this`), member (incl. `?.`), index (`a[i]`), array literals, unary `!` / `typeof` / `-` / `+`, ternary (`? :`), template literals (`` `a=${n}` `` / `` `${a + b}` ``), call, `new` (`K_NEW`), object literals, arrow (`(a,b)=>…` → `k8fn` arrow), `+=`/`-=`/`*=`/`/=`, `++`/`--`, let/var/assign (incl. member `this.n = n`), if/else, switch/case/default, while, for, for-in, for-of, break/continue, try/catch/finally, throw, function, `class` + constructor/`this` + `extends` (parent method merge), `await` (unwrap `__k8promise` / sync `.then`), async function (return `{__k8promise,value,then}`), `Promise.resolve` + `.then` microtask queue (`drainMicrotasks`), builtins: `Object.keys`/`assign`/`is`, `Symbol`/`Symbol.for`, `Array.isArray`, `Math.*`, `console.log`, `globalThis`, `document.createElement`/`body`/`appendChild`/`querySelector`/`getElementById`/`textContent`/`style`/`addEventListener`, `setTimeout`/`requestAnimationFrame`, binary: `+ - * / == === != !== < > <= >= && || ??` (short-circuit for `&&`/`||`/`??`).

**H6:** product path Kab-only. H6a ✅ delete Rust evalSource fallback. H6b ✅ subset query policy in `.kab`. See ROADMAP H6a–H6e. Gates: `kv8_h6_*`, `kv8_h6a_parity`, `h6b_query_policy`.
...
- Stack pop in `kv8/eval` uses native `pop` (C2). **`evalSource` / `evalSourceKab` → `evalSourceWith(source, null)`** (K1c–K1g + H6a Kabootar path). `preferKabEval()` kept as no-op compat. `evalSourceWith` stays self-host for `extraEnv`. Do not reintroduce Kabootar `evPopStack` rebuild loops.
